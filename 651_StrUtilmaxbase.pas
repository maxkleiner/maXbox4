...
interface
 //uses SysUtils,Classes; #sign:Max: MAXBOX10: 18/05/2016 09:36:48 
...
begin
 ...
 if Result>0 then   Inc1(Result,BeginPos-1)
end;
...
begin
  if N < 1 then Result := ''
    ...
    SetLength(Result, N);
    //StrFillChar(Result[1], Length(Result), C);
  end;
end;
...
begin
  ...
  try
   ...
   setLongDateFormat(Client_LongDateFormat);
   setdateseparator(Client_DateSeparator);
   setShortDateFormat(Client_ShortDateFormat);
   settimeseparator(Client_TimeSeparator);
   setShortTimeFormat(Client_ShortTimeFormat) // }
...
begin
  if (FLine = '*') or (SLine = '*') or (FLine=SLine) then result:= true
  else result:= false;
end;
...
begin
  ...
  if (Length(FLine)>0) and (Length(SLine)>0) then begin
      if    (FLine[1]<>'*') and (SLine[1]<>'*')
        ...
        and (FLine[1]<>SLine[1]) then result := true;
  end
  else begin
    if (Length(FLine)=0) and (SLine<>'*') then result := true;
    if (Length(SLine)=0) and (FLine<>'*') then result := true;
  end;
end;
  ...
  if (FLine[1]='*') then
   for k := 1 to Length(SLine) do
     if (SLine[k]='*') or (SLine[k]=FLine[2]) or (SLine[k]='?') then
       Res := Res or 
       wildcompare(Copy(FLine,2,Length(FLine)-1),    Copy(SLine,k,Length(SLine)-k+1));

  if (SLine[1]='*') then
   for k := 1 to Length(FLine) do
    if (FLine[k]='*')or(FLine[k]=SLine[2])or(FLine[k]='?') then
       Res  := Res
       or wildcompare(Copy(FLine,k,Length(FLine)-k+1), Copy(SLine,2,Length(SLine)-1));
...
begin
  ...
  result:= WildCompare(FirstString,SecondString);
end;
...
begin
  writeln(lastchar('maXbox4'))
  writeln(getdiroffile(exepath+Exename))
  
  writeln(itoa(DayOfYearfib(now)))
  writeln(itoa(DayOfYear(now)))
   writeln(itoa(DayOfMonth(now)))
  // writeln(itoa(DayOfMonth(now)))
  //RegisterOCX
  //RegisterServer
        //ClassIDToString({871C5380-42A0-1069-A2EA-08002B30309D});
        //ProgIDToClassID('{871C5380-42A0-1069-A2EA-08002B30309D}')
  ...
  if IsObjectActive('Explorer.Application') then ShowMessage('Explorer is running !');
  if IsObjectActive('InternetExplorer.Application') then 
                                      ShowMessage('IExplorer is running !');
  
  //if createguid2(aguid) = S_OK then writeln('guid create');

    // also internal as OpenIE()
    if OpenIEX('www.kleiner.ch') = true then writeln('ie ex now open');
...
End.
 ...
 //https://www.virustotal.com/en/file/ad0ab4407a2bd1558d4cbb1c1f8f090a131192d6510e0af4d48102ceccff1268/analysis/1453138546/
