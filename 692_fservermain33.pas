...
{**********************************************************************}
{}
{ $Log:  110838: fServer.pas #sign: Max: MAXBOX10: 18/05/2016 13:49:22 
{
...
{-----------------------------------------------------------------------------
 ...
 Author:    Allen O'Neill   Max Kleiner CancelBuffer size manager
...
if sCommand = 'SRN' then
  ...
  //  AThread.Connection.WriteFile(ExtractFileDir(ParamStr(0))+'\images\ScreenShot.BMP', true);
  ...
  if (sCommand <> 'LST') and (sCommand <> 'PIC') and (sCommand <> 'SRN') then
    Begin
    AThread.Connection.WriteLn('ERR : Unknown TCP command / action');
    AThread.Connection.Disconnect;
    end;
 except
//on E : Exception do
...
var
    ...
    S : String;
Begin
sFilePattern := ExtractFileDir(ParamStr(0)) + Imagepath+'*.bmp';
S := '';
...
//FreeAndNil(CS);
  ...
  writeln('free form and CS and TCPServer!')
end;
...
with frmserver do begin
  setbounds(355, 124, 348, 327)
  Caption := 'maXbox Indy - TCP Image Server'
  Color := clBtnFace
