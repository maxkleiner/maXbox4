...
//#headMMax: MAXBOX10: 18/05/2016 18:37:34 C:\Program Files (x86)\maxbox3\Import\IPC\maxbox4\maxbox4\examples\696_kmemo_demo_Main_test.pas 
...
procedure TForm1_FormCreateURLDetect(vrich: TRichEdit);
var
  s: string;
  //RichEdit1, RichEdit2: TRichEdit;
begin
  TForm1_InitRichEditURLDetection(vrich);
...
begin
    if EndTick >= StartTick then
        ...
        Result := High(StartTick) - StartTick + EndTick;
end;
 ...
 { afrm:= getForm(850,780) 
  afrm.borderstyle:= bssizeable; //bsdialog;
   ...
   //TForm1_InitRichEditURLDetectionK(kmemo);
    ...
    afrm:= getForm(850,780) 
  afrm.borderstyle:= bssizeable; //bsdialog;
   krich:= TRichedit.create(afrm); // do begin
   //TForm1_InitRichEditURLDetectionK(kmemo);
     ...
     font.size:= 14;
     //Options:= Options+ [eodropfiles1, eoShowFormatting, eoWantTab];
     TForm1_FormCreateURLDetect(krich)
     
     //TForm1_InitRichEditURLDetectionK(krich);
  
     align:= alClient;
     height:= height-50;
     //LoadFromRTF(Exepath+'\examples\kmemo_manual.rtf');
     //Blocks.AddHyperlink('  softwareschule','www.softwareschule.ch',1);
     showMessageBig('RichView versus KMemo in Action Box')
     free
     krich:= NIL;
    end;
    afrm.Release;
