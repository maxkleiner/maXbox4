{ Runtime part of the script engine }
unit uPSRuntime;
{$I PascalScript.inc}
{
...
bitmap canvas add by max , bintohex, hextobin , int64 , locs=11666
profiling of NO resourcestring at all! faster about 12%
}
...
interface
uses
  SysUtils, classes, uPSUtils{$IFDEF DELPHI6UP},
  variants{$ENDIF}{$IFNDEF PS_NOIDISPATCH}{$IFDEF DELPHI3UP}, 
  Windows{$ELSE}, Ole2, OleAuto{$ENDIF}{$ENDIF};
...
{ TPSError contains all possible errors }
...
@link(PIFProcRec)
@link(TPSList)
TPSProc is is the procedure definition of all external functions
}
...
{TPSProcRec contains a currently used internal or external function}
  TPSProcRec = class
  private
  ...
  public
    ...
    { attributes attached to this ProcRec }
...
{TProcrec is used to store an external function that could be used by the script executer}
  TProcRec = record
    { Name of this procrec. }
    Name: ShortString;
    { Hash of the name for this procrec. }
    Hash: Longint;
    { the pointer to the procedure that will be called. }
    ProcPtr: TPSProcPtr;
    { the procedure that will be called when this procrec is freed. }
    FreeProc: TPSFreeProc;
    { Ext1/Ext2 Pointer, for custom use }
    Ext1, Ext2: Pointer;
  end;
{@link(TBTReturnAddress)
  PBTReturnAddress is a pointer to an TBTReturnAddress record}
  PBTReturnAddress = ^TBTReturnAddress;
{ TBTReturnAddress is a record used to store function return
  information                                                }
  TBTReturnAddress = packed record
    { The Procedure Number of this return address. }
    ProcNo: TPSInternalProcRec;
    { the Stack base offset }
    Position, StackBase: Cardinal;
  end;
  { TPSTypeRec contains type info }
  TPSTypeRec = class
  private
    FExportNameHash: Longint;
    FExportName: string;
...
{$ENDIF}
  ...
  { A resource in PS is stored as a pointer to the proc and a tag (p) }
  TPSResource = record
    Proc: Pointer;
    P: Pointer;
  end;
  ...
  TPSVariantIFC = packed record // variants passed to InnerfuseCall
    Dta: Pointer;
    aType: TPSTypeRec;
    VarParam: Boolean;
  end;
  ...
  Protected
    { Contains the return address type. }
    FReturnAddressType: TPSTypeRec;
    { The Variant type. }
    FVariantType: TPSTypeRec;
    { The "Array of Variant" type. }
    FVariantArrayType: TPSTypeRec;
    { The attributes types registered to this runtime. }
    ...
    {List of SpecialProcs; See TPSExec.AddSpecialProc}
    ...
    ExParam: string;
    { \Internal function: Calls external proc pointers (ie event
      handlers)                                                  }
    ...
    { Find a special proc import, used for plugin libraries to
      register a special kind of procedures.                   }
  ...
  Public
    ...
    { Get a function number that has been exported with a name (by
      default)
                                                                   }
  ...
  {Contains the exception object}
    ...
    {Raises the current Exception object, or if that doesn't exist, an EPSException. If there
    is no current exception in pascal script at all, it does nothing}
  ...
  { stack type to store runtime information }
...
type
  { <TITLE ROPS Exception>
    Exception class for ROPS }
  ...
  private
    ...
    FExec: TPSExec;
  public
    { <TITLE constructor for this exception class.>
      
      Create a new instance of this class.          }
    constructor Create(const Error: string; Exec: TPSExec; Procno, ProcPos: Cardinal);
    { The internal proc number that caused this exception. }
    property ProcNo: Cardinal read FProcNo;
    { The binary position of the exception in this proc. }
    property ProcPos: Cardinal read FProcPos;
    { The runtime that caused this exception. }
    property Exec: TPSExec read FExec;
  end;
  {TPSRuntimeClass is one class at runtime}
  ...
  {TPSRuntimeClassImporter is the runtime class importer}
  ...
  public
    ...
    { Add a new item to this structure }
...
type
  ...
  {Alias to @link(uPSUtils.TPSCallingConvention)}
  TPSCallingConvention = uPSUtils.TPSCallingConvention;
const
  {Alias to @link(uPSUtils.cdRegister)}
  cdRegister = uPSUtils.cdRegister;
  {Alias to @link(uPSUtils.cdPascal)}
  cdPascal = uPSUtils.cdPascal;
  {Alias to @link(uPSUtils.cdCdecl)}
  cdCdecl = uPSUtils.cdCdecl;
  {Alias to @link(uPSUtils.cdStdCall)}
...
begin
  ...
  while Length(PName) > 0 do begin
    ...
    case pp^.PropType^.Kind of
      ...
      //after new pascalscript.inc + edefines.inc directives -PTypeInfo
...
begin
  case x of
    ...
    ErTypeMismatch: Result := 'TMismatch';
...
begin
  case aType.BaseType of
    ...
    btReturnAddress:; //no point in initializing a return address
    btRecord:
      begin
...
begin
  case aType.BaseType of
    ...
    btPointer:
      if Pointer(Pointer(IPointer(p)+8)^) <> nil then begin
...
var
  ...
  l: Longint;
begin
  for l := FAttributeTypes.Count -1 downto 0 do begin
...
begin
  for i := Longint(FExceptionStack.Count) -1 downto 0 do begin
    pp := FExceptionStack.Data^[i];
    Dispose(pp);
  end;
  for i := Longint(FResources.Count) -1 downto 0 do begin
...
begin
  ...
  for d := FExceptionStack.Count -1 downto 0 do begin
    ...
    if pp^.FinallyOffset <> InvalidVal then begin
      ...
      Exit;
    end else if pp^.ExceptOffset <> InvalidVal then begin
...
begin
  ...
  if LookupProc(FRegProcs, s) <> nil then begin
    Result:=  nil;
    exit;
  end;
  New(p);
...
var
  ...
  var
    ...
    begin
      ...
      if not Read(Name[1], NameLen) then begin
        CMD_Err(ErOutOfRange);
        Result := false;
        exit;
      end;
      if not Read(FieldCount, 4) then begin
        CMD_Err(ErOutOfRange);
        Result := false;
        exit;
      end;
      ...
      for i := 0 to FieldCount -1 do begin
        if (not Read(TypeNo, 4)) or (TypeNo >= Cardinal(FTypes.Count)) then begin
          CMD_Err(ErOutOfRange);
          Result := false;
          exit;
        end;
        ...
        case VarP^.FType.BaseType of
          ...
          bts8, btchar, btU8: if not read(PPSVariantU8(VarP)^.data, 1) then begin
              CMD_Err(erOutOfRange);
              DestroyHeapVariant(VarP);
              Result := False;
              exit;
          ...
          {$IFNDEF PS_NOINT64}
          bts64: if not read(PPSVariantS64(VarP)^.Data, sizeof(tbts64)) then begin
              CMD_Err(erOutOfRange);
              DestroyHeapVariant(VarP);
              Result := False;
              exit;
          ...
          btPchar, btString:
          begin
            if not read(NameLen, 4) then begin
                Cmd_Err(erOutOfRange);
                DestroyHeapVariant(VarP);
                Result := False;
                exit;
  ...
  begin
    if not Read(Count, 4) then begin
      CMD_Err(erOutofRange);
      Result := false;
      exit;
    end;
    for i := 0 to Count -1 do begin
      if not ReadAttrib then begin
        Result := false;
        exit;
      end;
    end;
      ...
      case currf.BaseType of
        ...
        btClass:
          begin
            ...
            if (not Read(d, 4)) or (d > 255) then begin
              curr.Free;
              cmd_err(erUnexpectedEof);
              LoadTypes := False;
              exit;
            ...
            if not Read(TPSTypeRec_Class(Curr).FCN[1], d) then begin
              curr.Free;
              cmd_err(erUnexpectedEof);
              LoadTypes := False;
              exit;
        ...
        btProcPtr:
          begin
            ...
            if not Read(TPSTypeRec_ProcPtr(Curr).FParamInfo[1], d) then begin
              curr.Free;
              cmd_err(erUnexpectedEof);
              LoadTypes := False;
              exit;
...
{$IFNDEF PS_NOINTERFACES}
        btInterface:
          begin
            ...
            if not Read(TPSTypeRec_Interface(Curr).FGUID, Sizeof(TGuid)) then begin
              curr.Free;
              cmd_err(erUnexpectedEof);
              LoadTypes := False;
              exit;
...
{$ENDIF}
        ...
        btStaticArray:
          begin
            ...
            {if not Read(d,4) then       //<-additional StartOffset bugy!!
            begin
              curr.Free;
              cmd_err(erUnexpectedEof);
              LoadTypes:=false;
            ...
            //TPSTypeRec_StaticArray(curr).StartOffset:= d-1;
        ...
        btRecord:
          begin
            ...
            while d > 0 do begin
              ...
              if Cardinal(l2) >= FTypes.Count then begin
                curr.Free;
                cmd_err(ErOutOfRange);
                LoadTypes := false;
                exit;
            ...
            if not resolve(TPSTypeRec_Record(curr)) then begin
              curr.Free;
              cmd_err(erInvalidType);
              LoadTypes := False;
              exit;
      ...
      if fe then begin
        ...
        if d > PSAddrNegativeStackStart then begin
          cmd_err(erInvalidType);
          LoadTypes := False;
          exit;
        end;
        SetLength(Curr.FExportName, d);
        if not read(Curr.fExportName[1], d) then begin
          cmd_err(erUnexpectedEof);
          LoadTypes := False;
          exit;
        end;
      ...
      curr.CalcSize;
      if HDR.PSBuildNo >= 21 then //build 21 we support attributes
      begin
...
{$WARNINGS ON}
  ...
  begin
    ...
    for l := 0 to HDR.VarCount - 1 do begin
      ...
      if (Rec.Flags and 1) <> 0then begin
...
//function load data for byte code builder
begin
  Clear;
  Pos := 0;
  LoadData := False;
...
begin
  if aType.BaseType = btPointer then begin
    ...
    Src := Pointer(Src^);
    if (src = nil) or (aType = nil) then raise Exception.Create('TMismatch');
  end;
  case aType.BaseType of
      ...
      case VarType(Variant(Src^)) of
        varString:
          if Length(VarToStr(Variant(Src^))) = 1 then
            ...
            raise Exception.Create('TMismatch');
{$IFNDEF PS_NOWIDESTRING}
        varOleStr:
...
{$ENDIF}
    ...
    else raise Exception.Create('TMismatch');
  end;
end;
...
{$IFNDEF PS_NOINT64}    btS64: tbts64(src^) := Val;
...
var
  o, i: Longint;
begin
  for i := 0 to aType.FieldTypes.Count -1 do begin
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                if Pointer(Pointer(IPointer(Src) + 8)^) = nil then begin
                  Pointer(Dest^) := Pointer(Src^);
      ...
      btProcPtr:
        begin
          if srctype.BaseType = btPointer then begin
            ...
            Src := Pointer(Src^);
            if (src = nil) or (srctype = nil) then raise Exception.Create('TMismatch');
          end;
...
{$IFDEF Delphi3UP}
          ...
          if srctype.BaseType = btClass then begin
    ...
    case Cmd of
      0: begin { >= }
          case var1Type.BaseType of
            ...
            btS32:
              begin
                if var2type.BaseType = btPointer then begin
                  ...
                  var2 := Pointer(var2^);
                  if (var2 = nil) or (var2type = nil) then raise Exception.Create('TMismatch');
                end;
            ...
            btVariant:
              begin
                if not IntPIFVariantToVariant(var2, var2type, tvar) then begin
                  Result := false;
                end else
                  b := Variant(var1^) >= tvar;
              end;
            btSet:
              begin
                if var1Type = var2Type then begin
      ...
      6: begin { in }
          if var2Type.BaseType = btSet then begin
            Cmd := PSGetUInt(var1, var1type);
            if not Result then begin
              CMD_Err(erTypeMismatch);
              exit;
            end;
            if Cmd >= Cardinal(TPSTypeRec_Set(var2Type).aBitSize) then begin
  ...
  case VarType of
    ...
    3:
      begin
        ...
        if UsePointer and (Dest.aType.BaseType = btPointer) then begin
          Dest.aType := TPSTypeRec(Pointer(IPointer(Dest.p)+4)^);
          Dest.P := Pointer(Dest.p^);
          if Dest.P = nil then begin
            Cmd_Err(erNullPointerException);
...
begin
  if FStatus <> isNotLoaded then begin
    ...
    if Cardinal(FStack.Count) > Cardinal(I) then begin
      vd := FStack[I];
      if (vd <> nil) and (vd.FType = FReturnAddressType) then begin
...
begin
  for i := Longint(FResources.Count) -1 downto 0 do begin
    if PPSResource(FResources[I])^.P = P then begin
      FResources.Delete(I);
      exit;
    end;
  end;
...
begin
  s := FastUpperCase(name);
  h := MakeHash(s);
  for l := FExportedVars.Count - 1 downto 0 do begin
    p := FexportedVars.Data^[L];
    if (p^.FNameHash = h) and(p^.FName=s) then begin
      Result := L;
      exit;
    end;
  end;
...
begin
  case Longint(p.Ext1) of         //3.6
    ...
    //procedure
    //62: BinToHex(pchar(Stack.getString(-2)),
      //                     pchar(Stack.getString(-3)), stack.getInt(-3));
     //function HexToBin(Text, Buffer: PChar; BufSize: Integer): Integer
    // max
    //66: System.Inc(Stack.GetInt(-1)); direct
    ...
    7: // StrGet
      begin
        ...
        if (i<1) or (i>length(tbtstring(temp.Dta^))) then begin
          Caller.CMD_Err2(erCustomError, 'OutOf StringRange');
          Result := False;
          exit;
        end;
...
{$IFNDEF PS_NOWIDESTRING}   //add to mX3
    42: // WStrGet
      begin
        temp :=  NewTPSVariantIFC(Stack[Stack.Count -2], True);
        if (temp.Dta = nil) or (temp.aType.BaseType <> btWideString) then begin
          Result := False;
          exit;
        end;
        I := Stack.GetInt(-3);
...
begin
  ...
  if arr.aType.BaseType=btString then begin
    SetLength(tbtstring(arr.Dta^),STack.GetInt(-2));
    Result:=true;
{$IFNDEF PS_NOWIDESTRING}
  end else
  if arr.aType.BaseType=btWideString then begin
...
function RealCall_CDecl(p: Pointer;
  StackData: Pointer;
  StackDataLen: Longint; // stack length in 4 bytes. (so 1 = 4 bytes)
  ResultLength: Longint; ResEDX: Pointer): Longint; Stdcall; // make sure all things are on stack
var
  r: Longint;
begin
...
function IntPIFVariantToVariant(Src: pointer; aType: TPSTypeRec; var Dest: Variant): Boolean;
  ...
  begin
    ...
    for i := 0 to Len -1 do
    begin
      if not IntPIFVariantToVariant(p, aType, v) then begin
        result := false;
        exit;
      end;
      Dest[i] := v;
...
begin
  ...
  if Ret.FEndOfVMT = MaxInt then begin
    ...
    if Ret.FEndOfVMT = MaxInt then begin
      Ret.FEndOfVMT := 0; // not find EndOfVMT
      Result := nil;
      exit;
    end;
  end;
...
begin
  Result := aVar;
  if Result.aType.BaseType = btArray then begin
    if (FieldNo <0) or (FieldNo >= PSDynArrayGetLength(Pointer(avar.Dta^), avar.aType)) then
    begin
      Result.Dta := nil;
      Result.aType := nil;
      exit;
...
begin
  ...
  try
    ...
    if tmp = nil then
      ...
      Caller.Cmd_Err3(erCustomError, 'Could notcall Proc', tmp);
    Result := false;
  end;
...
{$ENDIF}
    ...
    begin
      Caller.CMD_Err2(erCustomError, 'Cant cast interface');
      Result := False;
      exit;
    end;
{$IFDEF Delphi3UP}
...
begin
  s := p.Decl;
  delete(s, 1, 6);
  if s = '-' then begin {nil function}
    p.ProcPtr := NilProc;
    Result := True;
    exit;
  end;
  if s = '+' then begin {cast function}
    p.ProcPtr := CastProc;
    p.Ext2 := Tag;
    Result := True;
    exit;
  ...
  ISRead := False;
  cl := nil;
  for I := TPSRuntimeClassImporter(Tag).FClasses.Count -1 downto 0 do begin
    Cl := TPSRuntimeClassImporter(Tag).FClasses[I];
    if (Cl.FClassNameHash = h) and (cl.FClassName = s2) then begin
      IsRead := True;
      break;
    end;
  end;
  ...
  s2 := copy(S, 1, pos('|', s)-1);
  delete(s, 1, length(s2) + 1);
  if (s2 <> '') and (s2[length(s2)] = '@') then begin
    IsRead := False;
  ...
  p.Name := s2;
  H := MakeHash(s2);
  for i := cl.FClassItems.Count -1 downto 0 do begin
    px := cl.FClassItems[I];
    if (px^.FNameHash = h) and (px^.FName = s2) then begin
      p.Decl := s;
      case px^.b of
  {0: ext1=ptr}
...
begin
  Procno := GetProc(ProcName);
  if Procno = InvalidVal then begin
    Result.Code := nil;
    Result.Data := nil;
  end
  else
...
(*        else begin
    ...
    else if SupportsRegister(cpt) and (RegNo < 2) then begin
...
{$IFNDEF PS_NOINT64}
        if res^.FType.BaseType <> btS64 then
          CopyArrayContents(Pointer(Longint(FStack)-12), @PPSVariantData(res)^.Data, 1, Res^.FType);
{$ENDIF}
      end;
    end;
    DestroyHeapVariant(res);
...
begin
  h := MakeHash(Name);
  for i := FClasses.Count -1 downto 0 do begin
    p := FClasses[i];
    if (p.FClassNameHash = h) and (p.FClassName = Name) then begin
      Result := P;
      exit;
    end;
  end;
...
begin
  ...
  if not ok then raise Exception.Create('TMismatch');
end;
...
var
  ...
  I: Longint;
begin
  if Value < FLength then raise Exception.Create('Capacity be >= Length');
  ...
  try
    ...
    try
      for i := 0 to High(Par)  do begin
        if PVarData(@Par[High(Par)-i]).VType = varString then begin
