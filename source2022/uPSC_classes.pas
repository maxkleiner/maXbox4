...

{
  ...
  Register STD first   add delimiter & quotechar by max, linebreak, capacity , clear
   ...
   TTextReader  , assign TStrings
    ...
    add var streamable and more interfaces enumerators - solve exchange problem stringlist - CL.AddTypeS('RT_BITMAP','PChar(2)')
     after all a tlist  - rsesourcestream savetofile() 4.7.6}
...
procedure SIRegister_TList(CL: TPSPascalCompiler);
begin
  //with CL.RegClassS(CL,'TObject', 'TList') do    //to integer alias
  ...
  begin
    ...
    RegisterMethod('Procedure Error2( Msg : PResStringRec; Data : Integer);');
...
begin
  with Cl.AddClassN(cl.FindClass('TCUSTOMMEMORYSTREAM'), 'TResourceStream') do begin
    ...
    RegisterMethod('constructor Create1(Instance:THandle;ResName:String)');
    RegisterMethod('constructor Create2(Instance:THandle;ResName:String)');
...
begin
 ...
 CL.AddClassN(CL.FindClass('TOBJECT'),'TStrings');
  ...
  //CL.AddClassN(CL.FindClass('TOBJECT'),'TList');   //need for taligninfo - record AlignList : TList; ControlIndex : Integer; '
  //CL.AddClassN(CL.FindClass('Nil'),'TObject');
  //SIRegister_TList(CL);
...
begin
  ...
  if Streams then begin
   ...
   // CL.AddClassN(CL.FindClass('TOBJECT'),'TList');
  SIRegister_TList(CL);
  //CL.AddClassN(CL.FindClass('TList')
