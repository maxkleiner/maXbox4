...
Known Issues:         4.7.4.64         - 4.7.5.20
-----------------------------------------------------------------------------}
...
uses
   ...
   //{$R TestGPS.res}
    ...
    {$R extrares.res}
     ...
     {$R OpDesign.dcr}
      ...
      {$R SerDlgs.dcr}
     // {$R DepWalk.res}
      {$R CodeCompletion.dcr}
      {$R Fibplus.dcr}
      //{$R FIBComponents.RES}

      //{$R pkCindyCoreDXE.res}
      //{$R mandelbrot_ex.res}
      //{$R ExSend.res}
  ...
  //{$R DevCpp.res}
   ...
   //{$R WifiScanner.res}
   //{$R IWWizardNET.RES}
   //{$R DsgnCPortD2010.res}
 //  {$R dpkCindyDBXD2010.res}
  // {$R MSDesign.res}
   ...
   //{$R HexEditor.res}
   //{$R PascalCoinWallet.res}
   {$R DCPciphers.dcr}
   {$R DCPhashes.dcr}
   //{$R Ldap.res}
   {$R RvCsStd.res}
    {$R findcomputer.res}   // avi res 152
    {$R filehistogram3.res}
    {$R KhFunction.res}
    {$R yinyang.res}
    {$R yinyang3.res}
    {$R yingyang4.res}
