...
interface
 ...
 type
  ...
  TNode2= class(tObject) {A node of the graph}
    adjacents: tedgearray; //array of TEdge; {the edges to adjacent nodes}
  ...
  public
    ...
    function AddEdge(const node1,node2:TNode2; const newweight:integer):boolean; overload;
    function AddEdge(const node1,node2:TNode2; const newweight,newWeight2:integer):boolean; overload;

    {Find the edgerec connecting two given nodes (if it exists)}
    function findedge(const V1,V2:TNode2; var edgePtr:PTEdge):boolean;
...
begin
  {free all nodes}
  if count>0 then
  for i:=0 to count-1 do TNode2(objects[i]).free;
  edgecount:=0;  
  inherited;
end;
...
begin
  ...
  then
  begin
    node1:=TNode2(objects[k1]);
    node2:=TNode2(objects[k2]);
    result:=addedge(node1,node2, newweight,newweight2);
  end;
end;

{*************** AddEgde (By node, (directed one-way) ****}
function TGraphlist.AddEdge(const Node1,Node2:TNode2; const newweight:integer):boolean;
begin
...
function TGraphlist.AddEdge(const node1,node2:TNode2; const newweight,newWeight2:integer):boolean;
var
  j:integer;
  found:boolean;
  k1,k2:integer;
...
{*************** FindEdge ************}
function TGraphlist.Findedge(const V1,V2:TNode2; var edgePtr:PTEdge):boolean;
...
begin
  result:=false;
  with edge do
  with TNode2(objects[fromnodeindex]) do
  begin
    for i:= 0 to nbradjacents-1 do
...
begin
  ...
  begin
    ...
    with adjacents[j] do
    begin
      temp:=TNode2(objects[tonodeindex]);
...
var
 ...
 begin
   ...
   if stop then exit;
   inc(nodesSearched);
   temp:=TNode2(objects[v]);  {get the node}
...
begin
  ...
  try
    ...
    temp.node:=TNode2(objects[nodenbr]);
    temp.pathlength:=1;
      ...
      begin {go through adjacents and make new breadth objects for them}
        ...
        begin
          ...
          begin
            ...
            with temp2 do
            begin
              node:=TNode2(objects[temp.node.adjacents[i].ToNodeIndex]);
            ...
            begin
              ...
              begin
                node2:=TNode2(objects[temp2.path[j]]);
                q.add(node2);
              end;
...
begin
  ...
  begin
    ...
    for i:=extpath.count-1 downto 0 do
          q.add(TNode2(objects[extpath[i]]));
    queuesize:=q.count;
    goalfound;
  end
