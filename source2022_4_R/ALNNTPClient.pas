...
Begin
  ...
  aResponse := StringReplace(aResponse,#9,' ',[RfReplaceAll]); // 211 111225 12861 362539 nzn.fr.delphi
  While alPos('  ',aResponse) > 0 do aResponse := StringReplace(aResponse,'  ',' ',[RfReplaceAll]); // 211 111225 12861 362539 nzn.fr.delphi
  aResponse := ALTrim(StringReplace(aResponse,' ',#13#10,[RfReplaceAll])); // 211
                                                                           // 111225
                                                                           // 12861
                                                                           // 362539
                                                                           // nzn.fr.delphi
...
begin
  Result := SendCmd('NEWGROUPS ' + FormatDateTime('yymmdd" "hhnnss',FromGMTDate, ALDefaultFormatSettings) + ' GMT' + ALNNTPClientEnclosedInAngleBrackets(distributions), [231], True);
end;
...
begin
  Result := SendCmd('NEWNEWS ' + newsgroups + ' ' + FormatDateTime('yymmdd" "hhnnss',FromGMTDate, ALDefaultFormatSettings) + ' GMT' + ALNNTPClientEnclosedInAngleBrackets(distributions), [230], True);
end;
