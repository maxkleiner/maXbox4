...
const
  UnitVersioning: TUnitVersionInfo = (
    RCSfile: '$URL: https://jcl.svn.sourceforge.net/svnroot/jcl/tags/JCL-1.102-Build3072/jcl/source/common/JclBorlandTools.pas $';
...
uses
  ...
  Libc,
  {$ENDIF HAS_UNIT_LIBC}
  JclFileUtils, JclLogic, JclResources_max, JclStrings, JclWideStrings, JclSysInfo, JclSimpleXml;
...
begin
  ...
  begin
    ...
    Result := JclSysUtils_max.Execute(LaunchCommand, FOutputCallback) = 0;
  end
  else
    Result := JclSysUtils_max.Execute(LaunchCommand, FOutput) = 0;
end;
...
begin
  if IDEVersionNumber >= 5 then
  begin
    if (RootKey = 0) or (RootKey = 0) then
...
begin
  if Assigned(FOutputCallback) then
    Result := JclSysUtils_max.Execute(Format('"%s" %s', [ExeName, CommandLine]), FOutputCallback) = 0
  else
    Result := JclSysUtils_max.Execute(Format('"%s" %s', [ExeName, CommandLine]), FOutput) = 0;
end;
