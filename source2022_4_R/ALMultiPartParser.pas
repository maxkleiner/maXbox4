...
begin
  ...
  if CountOfCRLF > 0 then begin
    ...
    CurrentPos := 0;
    for I := 0 to CountOfCRLF - 1 do begin     // fix it
...
begin
  ...
  Try
    ...
    For i := 0 to LLst.Count - 1 do
      If SameText(ALTrim(LLst.Names[i]), aName) then begin
...
begin
  ...
  //result := ALBase64DecodeString(StringReplace(s, #13#10, '', [rfReplaceAll]));
...
begin
  Result := '---------------------------' + FormatDateTime('mmddyyhhnnsszzz', Now, ALDefaultFormatSettings);
end;
...
begin
  LStringStream:= TStringStream.Create(aFieldValue);
  LContent := TALMultiPartFormDataContent.Create;
  Try
...
begin
  ...
  LFilenameWithoutPath := ExtractFileName(aFileName);
