...
type
  ...
  {------------------------------------------}
  EALOpenOfficeException = class(Exception);
...
function CreateWordDocument: Variant;
function CreateImpressDocument: Variant;
function CreateDrawDocument: Variant;
...
begin
  ...
  if (not Odd(aNumberOfProperties)) or (aNumberOfProperties < 1) then
    raise EALOpenOfficeException.CreateFmt('Wrong number of properties: %s', [IntToStr(aNumberOfProperties)]);
...
begin
  aArgs := VarArrayCreate([0, 0], varVariant);
  result := vALOpenOfficeStarDesktop.LoadComponentFromURL('private:factory/swriter',
                                                          '_blank',
                                                          0,
                                                          aArgs);
end;
...
begin
  aArgs := VarArrayCreate([0, 0], varVariant);
  result := vALOpenOfficeStarDesktop.LoadComponentFromURL('private:factory/simpress',
                                                          '_blank',
                                                          0,
                                                          aArgs);
end;
...
begin
  aArgs := VarArrayCreate([0, 0], varVariant);
  result := vALOpenOfficeStarDesktop.LoadComponentFromURL('private:factory/sdraw',
                                                          '_blank',
                                                          0,
                                                          aArgs);
end;
...
//var ALMove := system.Move;

// Taken from https://github.com/synopse/mORMot.git
  // https://synopse.info
  // http://mormot.net

function ALCopyStr2X(const aSourceString: AnsiString; aStart, aLength: Integer): AnsiString;
...
begin
  ...
  System.Move(Pbyte(aSourceString[aStart-1])^, pointer(Result)^, aLength); // pointer(Result)^ to not jump inside uniqueString (aDestString is already unique thanks to previous SetLength))
end;
...
var aFileContentService: Variant;
    ...
    i: integer;
begin
  aLowWinAddress := LowerCase(aWinAddress);
  aPrefix := '';
  for i := 1 to High(cALOpenOfficeUrlProtocols) do begin
    if Pos(cALOpenOfficeUrlProtocols[i], aLowWinAddress) = 1 then begin
      aWinAddress := AlCopyStr2(aWinAddress, Length(cALOpenOfficeUrlProtocols[i]) + 1, maxint);
  ...
  if (Length(aPrefix) = 0) and
     (Pos('@', aLowWinAddress) > 0) then begin
...
var aFileContentService: Variant;
    ...
    i: integer;
begin
  aLowUrlAddress := LowerCase(aUrlAddress);
  aPrefix := '';
  for i := 1 to High(cALOpenOfficeUrlProtocols) do begin
    if Pos(cALOpenOfficeUrlProtocols[i], aLowUrlAddress) = 1 then begin
      if i > 1 then begin
        aUrlAddress := ALCopyStr2(aUrlAddress, Length(cALOpenOfficeUrlProtocols[i]) + 1, maxint);
  ...
  if Length(aPrefix) <> 0 then // backslash only with file:///
    aWinAddress := StringReplace(aWinAddress, '\', '/', [rfReplaceAll]);
