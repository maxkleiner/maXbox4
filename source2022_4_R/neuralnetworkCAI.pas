...
type
  ...
  TNNet = class(TMObject)
    ...
    public
      ...
      function AddLayerAfter1(pLayer: TNNetLayer; pAfterLayerIdx: integer): TNNetLayer; //overload;
      ...
      procedure Compute1(pInput: array of TNNetVolume);
  ...
  THistoricalNets = class(TNNet)
    public
      ...
      function AddDenseNetBlockCAI70(pUnits, k, supressBias: integer;
  ...
  TNNetForByteProcessing = class(TNNet)
    ...
    public
      ...
      procedure Compute2(var pInput: TBytes);
      procedure Backpropagate(var pOutput: array of byte);
      procedure Backpropagate2(var pOutput: TBytes);
      procedure GetOutput(var pOutput: array of byte);
      procedure GetOutput2(var pOutput: TBytes);
  end;
...
uses fmain; //maxForm1.memo2.lines.Add('
...
begin
  ...
  begin
    maxForm1.memo2.lines.Add(

      'Sizes differ. Current layer: '+inttostr(CurrentLayer.Count)+
      ' Calc layer: '+inttostr(CalculatedLayer.Count));
    exit;
  end;
...
procedure TNNetForByteProcessing.Compute2(var pInput: TBytes);
begin
  FInput.CopyAsBits(pInput, -0.5, +0.5);
  inherited Compute(FInput);
end;
...
procedure TNNetForByteProcessing.Backpropagate2(var pOutput: TBytes);
begin
  FOutput.CopyAsBits(pOutput);
  inherited Backpropagate(FOutput);
end;
...
procedure TNNetForByteProcessing.GetOutput2(var pOutput: TBytes);
begin
  inherited GetOutput(FOutput);
  FOutput.ReadAsBits(pOutput);
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        PtrA := OutputPtr;
        {$ENDIF}
          //maxForm1.memo2.lines.Add(inttostr(InputX)+' '+inttostr( InputY)+' '+inttostr(OutputX)+' '+inttostr(OutputY));
...
begin
  ...
  //maxForm1.memo2.lines.Add('norm debug '+floattostr(FOutput.GetAvg())+' '+floattostr(StdDevError)+' '+floattostr(CurrentStdDev));
  if StdDev > 1 then
  begin
    // The direction of the error is more important than its magnitude.
...
function THistoricalNets.AddDenseNetBlockCAI70(pUnits, k, supressBias: integer;
  PointWiseConv: TNNetConvolutionLinear;
  IsSeparable: boolean = false;
  HasNorm: boolean = true;
  pBefore: TNNetLayerClass = nil;
...
var
  ...
  AllGood: boolean;
begin
  maxForm1.memo2.lines.Add('Testing Data Parallelism...');

  Par := TNNetDataParallelism.Create(NN, 4);
  maxForm1.memo2.lines.Add('TTNNetDataParallelism.Create(NN, 4) passed...');   //fix
  NN2 := NN.Clone();
  Par.SumWeights(NN2);
  AllGood := True;
  ...
  begin
    ...
    begin
      maxForm1.memo2.lines.Add('Data Parallelism Error: neuron count doesn''t match on layer:'+inttostr(I));
      AllGood := False;
    end;

    if NN.Layers[I].CountWeights() <> NN2.Layers[I].CountWeights() then
    begin
      maxForm1.memo2.lines.Add('Data Parallelism Error: weight count doesn''t match on layer:'+inttostr(I));
      AllGood := False;
    end;

    if 4 * NN.Layers[I].GetWeightSum() <> NN2.Layers[I].GetWeightSum() then
    begin
      maxForm1.memo2.lines.Add('Data Parallelism Error: weight sum doesn''t match on layer:'+inttostr(I));
      AllGood := False;
    end;

    if 4 * NN.Layers[I].GetBiasSum() <> NN2.Layers[I].GetBiasSum() then
    begin
      maxForm1.memo2.lines.Add('Data Parallelism Error: bias sum doesn''t match on layer:'+inttostr(I));
      AllGood := False;
    end;

    if 4 * NN.Layers[I].GetInertiaSum() <> NN2.Layers[I].GetInertiaSum() then
    begin
      maxForm1.memo2.lines.Add('Data Parallelism Error: inertial sum doesn''t match on layer:'+inttostr(I));
      AllGood := False;
    end;
  end;
  if AllGood
  then maxForm1.memo2.lines.Add('Data Parallelism testing has been very passed.');
  NN2.Free;
  Par.Free;
end;
...
var
  ...
  AllGood: boolean;
begin
  maxForm1.memo2.lines.Add('Structural Test Start');
  AllGood := True;

  if NN.SaveToString() <> NN2.SaveToString() then
  begin
    maxForm1.memo2.lines.Add('Error: network save to string differs.');
    AllGood := False;
  end
  else
  begin
    maxForm1.memo2.lines.Add('Saving to string has passed.');
  end;
  ...
  begin
    ...
    begin
      maxForm1.memo2.lines.Add('Error: save data to string doesn''t match on layer:'+inttostr(I));
      AllGood := False;
    end;

    if NN.Layers[I].SaveStructureToString() <> NN2.Layers[I].SaveStructureToString() then
    begin
      maxForm1.memo2.lines.Add('Error: save structure to string doesn''t match on layer:'+inttostr(I));
      maxForm1.memo2.lines.Add('NN Structure:'+ NN.Layers[I].SaveStructureToString());
      maxForm1.memo2.lines.Add('NN2 Structure:'+ NN2.Layers[I].SaveStructureToString());
      AllGood := False;
    end;

    if NN.Layers[I].ClassName <> NN2.Layers[I].ClassName then
    begin
      maxForm1.memo2.lines.Add('Error: class name doesn''t match on layer:'+inttostr(I));
      AllGood := False;
    end;

    if NN.Layers[I].Neurons.Count <> NN2.Layers[I].Neurons.Count then
    begin
      maxForm1.memo2.lines.Add('Error: neuron count doesn''t match on layer:'+inttostr(I));
      AllGood := False;
    end;

    if NN.Layers[I].CountWeights() <> NN2.Layers[I].CountWeights() then
    begin
      maxForm1.memo2.lines.Add('Error: weight count doesn''t match on layer:'+inttostr(I));
      AllGood := False;
    end;

    if NN.Layers[I].GetWeightSum() <> NN2.Layers[I].GetWeightSum() then
    begin
      maxForm1.memo2.lines.Add('Error: weight sum doesn''t match on layer:'+inttostr(I));
      AllGood := False;
    end;
  end;
  if AllGood
  then maxForm1.memo2.lines.Add('Structural testing has passed.');
end;
...
begin
  NN := TNNet.Create;
  maxForm1.memo2.lines.Add('Structural Test Backpropagation starts.');
  (*
  NN.AddLayer([
  ...
  *)
  ...
  OutputVolume.Free;
  InputVolume.Free;
  maxForm1.memo2.lines.Add('Structural Test Backpropagation has passed.');
end;
...
begin
  ...
  if SumDiff <> 0 then
  begin
    maxForm1.memo2.lines.Add('Forward computing FAILS: '+floattostr( SumDiff));
    maxForm1.memo2.lines.Add('Output sum 1: '+floattostr( OutputVolume1.GetSum()));
    maxForm1.memo2.lines.Add('Output sum 2: '+floattostr( OutputVolume2.GetSum()));
    //maxForm1.memo2.lines.Add('debug diff '+OutputVolume1.DebugDiff(OutputVolume2, 0.1);
    OutputVolume1.DebugDiff(OutputVolume2, 0.1);
  end
  else
  begin
    maxForm1.memo2.lines.Add('Forward computing has passed.');
  end;
  ...
  if SumDiff <> 0 then
  begin
    maxForm1.memo2.lines.Add('Backprop error FAILS:'+floattostr( SumDiff));
  ...
  begin
    maxForm1.memo2.lines.Add('Backpropagation computing has passed.');
  end;
...
begin
  ...
  AuxVolume := TNNetVolume.Create;
  maxForm1.memo2.lines.Add('TestConvolutionAPI(); started...');
  ...
  //WriteLn('Test Grouped Convolution:');
   maxForm1.memo2.lines.Add('Test Grouped Convolution:started...');
  ...
  begin
    //WriteLn(I, ' weights:');
    maxForm1.memo2.lines.Add(inttostr(I)+ 'weights:');
  ...
  maxForm1.memo2.lines.Add('Output:');
  AuxVolume.PrintWithIndex();

  AuxVolume.FillForDebug();AuxVolume.Mul(100);
  NN.Backpropagate(AuxVolume);

  maxForm1.memo2.lines.Add('Error at grouped conv:');
  NN.Layers[1].OutputError.PrintWithIndex();

  maxForm1.memo2.lines.Add('Error at input:');
  NN.Layers[0].OutputError.PrintWithIndex();
  //*)

  maxForm1.memo2.lines.Add('Test DeMaxPool:');
  ...
  maxForm1.memo2.lines.Add('MaxPool Output:');
  AuxVolume.PrintWithIndex();
  ...
  NN.Compute(AuxVolume);
  NN.GetOutput(AuxVolume);
  maxForm1.memo2.lines.Add('Convolution Output:');
  AuxVolume.PrintWithIndex();

  maxForm1.memo2.lines.Add('Test MaxPool:');
  ...
  //WriteLn;
  //*)
  maxForm1.memo2.lines.Add('Complete CNN Convolution API Testing has finished.');
  AuxVolume.Free;
  NN.Free;
  NN2.Free;
end;
...
begin
  ...
  NNData := CloneNN.SaveToString();       //fix
  maxForm1.memo2.lines.Add('Debug Fit TNNetDataParallelismCloneLen: '+inttostr(length(NNData)));
  ...
  maxForm1.memo2.lines.Add('Debug Fit NN.LoadFromString: '+inttostr((NN.CountLayers)));
end;
...
procedure TNNetConvolution.ComputeTiledCPU();     //add fix  takes multiple time
begin
...
begin
  ...
  S2 := CreateTokenizedStringList(strData,';');    //fix 8
  ...
  begin
    ...
    {$ELSE}
      maxForm1.memo2.lines.Add('debug createLayer():'+s[0]+ '-'+addlayeraftertxt);
      //maxForm1.memo2.lines.Add('debug createLayerSt():'+inttostr(st[0])+ ' '+inttostr(st[1])+ ' '+inttostr(st[2]));
      ...
      if S[0] = 'TNNetPoolBase' then Result := TNNetMaxPool.Create(St[0], St[1], St[2]) else
    ...
    begin
      ...
      begin
       // WriteLn('AddAutoGroupedPointwiseConv - Bad input channel counts:',
         ...
         maxForm1.memo2.lines.Add('etLastLayer():'+inttostr(GetLastLayer().Output.Depth)));
       // );
      end;
      {$ENDIF}
...
function TNNet.AddLayerAfter1(pLayer: TNNetLayer; pAfterLayerIdx: integer
  ): TNNetLayer;
begin
  pLayer.NN := Self;
  if pAfterLayerIdx >= 0 then
...
begin
  //maxForm1.memo2.lines.Add('AddLayerAfter01...'+strData);
  addlayeraftertxt:= ('addLayerAfter'+inttostr(pAfterLayerIdx)+'...'+strData);
  Result := AddLayerAfter1(CreateLayer(strData), pAfterLayerIdx);
  //maxForm1.memo2.lines.Add('AddLayerAfter02...'+strData);
end;
...
begin
  ...
  begin
    ...
    FLayers[LastLayer].Backpropagate();    //debug -1 out!
    ComputeL2Decay();
  end else
  begin
    ...
    begin
      ...
      begin
        maxForm1.memo2.lines.Add(
          ...
          ' Neurons:'+inttostr(NeuronCount)+
          ' Max Weight: '+floattostr(FLayers[LayerCnt].GetMaxWeight())+
          ' Min Weight: '+floattostr(FLayers[LayerCnt].GetMinWeight())+
          ' Max Output: '+floattostr(FLayers[LayerCnt].Output.GetMax())+
          ' Min Output: '+floattostr(FLayers[LayerCnt].Output.GetMin())+
          ' ' + FLayers[LayerCnt].ClassName + ' ' +
      ...
      maxForm1.memo2.lines.Add(
          ...
          ' Times: '+
          floattostr(FLayers[LayerCnt].FForwardTime  * 24 * 60 * 60)+ 's '+
          floattostr(FLayers[LayerCnt].FBackwardTime * 24 * 60 * 60)+'s' );

      if Assigned(FLayers[LayerCnt].PrevLayer) then
      begin
        maxForm1.memo2.lines.Add(' Parent:'+inttostr(FLayers[LayerCnt].PrevLayer.LayerIdx));
      end
      else
      begin
        //WriteLn;
        maxForm1.memo2.lines.Add('debug weights end.....')
      end;
...
begin
  ...
  begin
    ...
    begin
        ...
        //(
          'Layer '+IntToStr(LayerCnt)+
          ' Max Error: '+floattostr(FLayers[LayerCnt].OutputError.GetMax())+
          ' Min Error: '+floattostr( FLayers[LayerCnt].OutputError.GetMin())+
          ' Max ErrorD: '+floattostr(FLayers[LayerCnt].OutputErrorDeriv.GetMax())+
          ' Min ErrorD: '+floattostr(FLayers[LayerCnt].OutputErrorDeriv.GetMin())+
          ' ' + FLayers[LayerCnt].ClassName + ' ' +
          IntToStr(FLayers[LayerCnt].Output.SizeX) + ',' +
          IntToStr(FLayers[LayerCnt].Output.SizeY) + ',' +
          IntToStr(FLayers[LayerCnt].Output.Depth)
      ...
      begin
        maxForm1.memo2.lines.Add( 'debug errors else')
      end;
...
begin
  maxForm1.memo2.lines.Add(' Layers: '+inttostr( CountLayers()));
  maxForm1.memo2.lines.Add(' Neurons: '+inttostr( CountNeurons()));
  maxForm1.memo2.lines.Add(' Weights: '+inttostr( CountWeights()));
  maxForm1.memo2.lines.Add(' Sum: '+floattostr( GetWeightSum()));
    ...
    begin
      ...
      maxForm1.memo2.lines.Add
        (
          ...
          IntToStr(FLayers[LayerCnt].Output.SizeY) + ',' +
          IntToStr(FLayers[LayerCnt].Output.Depth)+
          ' Learning Rate:'+Format('%6.4f ',[FLayers[LayerCnt].LearningRate])+
          ' Inertia:'+Format('%4.2f ',[FLayers[LayerCnt].Inertia])+
          ' Weight Sum:'+Format('%8.4f ',[FLayers[LayerCnt].GetWeightSum()])
        );
     { Write     Format('AccuracyWI %6.4f ',[AccuracyWithInertia/100])+','+
        (
          'Layer ',LayerCnt:2,
      ...
      maxForm1.memo2.lines.Add(' Branches:'+inttostr(FLayers[LayerCnt].FDepartingBranchesCnt));
...
begin
  ...
  Result := S.DelimitedText;
   //maxForm1.memo2.lines.Add('Debug datatostring: '+result);
   S.Free;
end;
...
begin
  S := CreateTokenizedStringList('#');
  maxForm1.memo2.lines.Add('Debug SavetokenStructureToString: '+s.text);
  ...
  begin
    ...
    begin
      ...
      S.Add( IntToStr(PrevLayerIdx) + ')' + FLayers[LayerCnt].SaveStructureToString() );       //fix
    end;
  end;
  Result := S.DelimitedText;
  maxForm1.memo2.lines.Add('Debug structuretostring: '+result);
  S.Free;
end;
...
begin
  ...
  //maxForm1.memo2.lines.Add('Debug TNNet.Struct.LoadFromString S: '+inttostr(S.Count));
  maxForm1.memo2.lines.Add('Debug TNNet.Struct.LoadFromString ST: '+#13#10+S.text);
  //maxForm1.memo2.lines.Add('Debug TNNet.Struct.LoadFromString S2: '+inttostr(S2.Count));
  if S.Count > 0 then
  begin
...
begin
  ...
  begin
    ...
    maxForm1.memo2.lines.Add('Debug Struct.LoadFromString: '+inttostr(S.Count));
    LoadDataFromString(S[1]);
    maxForm1.memo2.lines.Add('Debug Data.LoadFromString: '+inttostr(S.Count));
  end
  else
  begin
    FErrorProc
...
begin
  S := CreateTokenizedStringList(strData,'!');
  maxForm1.memo2.lines.Add('Debug TNNet Loaded Data Layers:'+IntToStr(FLayers.Count)+'  SCount:'+IntToStr(S.Count));
  ...
  begin
    ...
    {$IFDEF Debug}
    maxForm1.memo2.lines.Add('Loaded Data Layers:');
    if S.Count > 0 then
    begin
      for Cnt := 0 to S.Count - 1 do
      begin
        maxForm1.memo2.lines.Add(inttostr(Cnt)+ ':'+floattostr( Copyf(S[Cnt]) );
      end;
    end;

    //WriteLn('Structure Layers:');
    maxForm1.memo2.lines.Add('Structure Layers::');
    if FLayers.Count>0 then
    begin
      for Cnt := 0 to FLayers.Count - 1 do
      begin
        maxForm1.memo2.lines.Add(inttostr(Cnt)+ ':'+ FLayers[Cnt].ClassName );
      end;
    end;
    {$ENDIF}
  end;
...
begin
  S := CreateTokenizedStringList(strData,'[');
  //maxForm1.memo2.lines.Add('Debug TNNetLayer Loaded NeuronsLayers:'+IntToStr(FNeurons.Count)+'  StrCount:'+IntToStr(S.Count));
...
begin
  ...
  for I := Low(FStruct) to High(FStruct) do      //fix always 8 false
  begin
    if I > 0 then Result := Result + ';';
...
begin
  ...
  if (CacheSize>0)
  then FCache.Init(pActionByteLen, pStateByteLen) else
     FCache.Init(1, 1);
...
procedure TNNet.Compute1(pInput: array of TNNetVolume);
//begin
  //Self.Compute(pInput, FromLayerIdx);
  //Self.GetOutput(pOutput);
  var
...
begin
  ...
  begin
    ...
    maxForm1.memo2.lines.Add(
      'Prepared input Depth doesn''t match weight size at neuron '+inttostr(NeuronIdx)+':'+
      'Weight Size:'+inttostr( LocalW.Size)+
      'Depth:'+inttostr(FInputPrepared.Depth);  //}
  end;
  {$ENDIF}
