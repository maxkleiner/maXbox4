...
(**************************************************************************)
// changes for maXbox4 integration tester frame   - dll run exception
// PyUnicode_Check error  --> pyUnicodeAsUTF8String( obj : PPyObject ) : RawByteString;
// check encodestring and cleanstring
...
//#######################################################
  ...
  // Standard exception classes of Python and maXbox
...
type
  ...
  TSendUniDataEvent = procedure (Sender: TObject; const Data : String ) of object;
  TReceiveUniDataEvent = procedure (Sender: TObject; var Data : String ) of object;
  IOChar = WideChar;
...
begin
  ...
  begin
    ...
    try
      ...
      {$ELSE}
      FDLLHandle := LoadLibraryEx(PAnsichar(GetDllPath+DllName),
      {$ENDIF}
...
begin
  ...
  try
    ...
    //Result := PyRun_String(PAnsiChar((command)), mode, _globals, _locals);
    if Result = nil then
      CheckError(False);
  except
    if PyErr_Occurred <> nil then
...
{  if PyUnicode_Check(obj) then
 ...
 if Assigned(S) then
  begin
    ...
    //W:= PyUnicodeAsUTF8String(S) ;
    //W:= PyBytes_AsString(S);
    W:= PyUnicodeAsUTF8String(S);
    result:= string(W);
    //Result := string(W);
  end;  //}
  //result:= S;
  //result:= PyObjectAsString(S)
  Py_XDECREF(S);
end;
...
var
  ...
  size: NativeInt;
begin
  //if PyUnicode_Check(obj) then   fix
  if assigned(obj) then
  begin
    Result := '';
    ...
    if Assigned(buffer) then
      ...
      Result := '';
    //SetCodePage(Result, CP_UTF8, False);      fix
  end
  else
...
begin
  ...
  //FillChar( FMethods[MethodCount+1] ,SizeOf(PyMethodDef)*PYT_METHOD_BUFFER_INCREASE,0);    fix
end;
...
begin
  ...
  //FillChar( FMembers[MemberCount+1], SizeOf(PyMemberDef)*PYT_MEMBER_BUFFER_INCREASE,0);  fix
end;
...
begin
  ...
  //FillChar( FGetSets[GetSetCount+1], SizeOf(PyGetSetDef)*PYT_GETSET_BUFFER_INCREASE,0);   fix
end;
    ...
    begin
      if Assigned(args) and (PyTuple_Size(args) > 0) then
        begin
          ...
          begin
            {if PyUnicode_Check(a1) then           fix for IO fix
