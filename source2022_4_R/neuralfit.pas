...
51 Franklin Street, Fifth Floor, Boston, MA 02110-1301 USA.  check the writeln()
create with override - check all readln()*)
unit neuralfit;
...
uses
  Classes, SysUtils, neuralnetworkCAI, neuralvolume, neuralthread, neuraldatasets
  {$IFDEF OpenCL}, cl
...
type
  ...
  TNeuralFitWithImageBase = class(TNeuralFitBase)
    ...
    public
      ...
      procedure CloseResFile; virtual;
...
procedure TNeuralFitWithImageBase.CloseResFile;
begin
  //CloseFile(CSVFile);
end;
...
procedure TNeuralDataLoadingFit.CloseResFile;
begin
  //CloseFile(CSVFile);
  CloseFile(FCloseFile);
end;
...
begin
  ...
  //FCloseFile:= AssignFile(CSVFile); //addr(CSVFile);
  //AssignFile(FCloseFile, CSVFile^);
  FMaxEpochs := Epochs;
  ...
  //maxForm1.memo2.lines.Add(CSVFile+
  //'//epoch,training accuracy,training loss,training error,validation accuracy,validation loss,validation error,learning rate,time,test accuracy,test loss,test error');
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        MessageProc('Image mX4 FThreadNN[0].DebugWeights(); skipped...');
      end;
      ...
      if ( (FCurrentEpoch mod 10 = 0) and (FCurrentEpoch > 0) ) then
      begin
     (* maxForm1.memo2.lines.Add(string(CSVFile) +
        'FCurrentEpoch '+inttostr(FCurrentEpoch)+','+
        Format('AccuracyWI %6.4f ',[AccuracyWithInertia/100])+','+
          TrainingLoss:6:4,',',
          TrainingError:6:4,',',
          ValidationRate:6:4,',',
          ValidationLoss:6:4,',',
  ...
  CloseFile(CSVFile);
  FMessageProc('CAI Constructor maXbox Proc Finished.');
  FreeMemory();
end;

procedure TNeuralDataLoadingFit.FitLoading2(pNN: TNNet; TrainingCnt,
  ValidationCnt, TestCnt, pBatchSize, Epochs: integer;
  pGetTrainingPair, pGetValidationPair,
  pGetTestPair: TNNetGetPairFn);
var
...
begin
  AllocateMemory(pNN, pBatchSize, pGetTrainingPair,
    pGetValidationPair, pGetTestPair);
  maxForm1.memo2.lines.Add('Debug MemAlloc: '+inttostr(Epochs));
  FMaxEpochs := Epochs;
  AccuracyWithInertia := 0;
  TrainingError := 0;
  TrainingLoss := 0;
  ...
  if FVerbose then
  begin
    MessageProc('Fit2 Computing...'+IntToStr(TrainingCnt));
  end;
  if Assigned(FOnStart) then FOnStart(Self);
  globalStartTime := Now();
  while ( (FMaxEpochs > FCurrentEpoch) and Not(FShouldQuit) ) do
  begin
    ...
    startTime := Now();
    CheckLearningRate(FCurrentEpoch);
    maxForm1.memo2.lines.Add('Debug CheckLearningRate Epoch: '+inttostr(FCurrentEpoch));
    for I := 1 to (TrainingCnt div FStepSize) {$IFDEF MakeQuick}div 10{$ENDIF} do
    begin
      if FShouldQuit then break;
      RunTrainingBatch();
      maxForm1.memo2.lines.Add('Debug RunTrainingBatch Epochs: '+inttostr(Epochs));
...
procedure TNeuralDataLoadingFit.FitLoading22(pNN: TNNet; TrainingCnt,
  ValidationCnt, TestCnt, pBatchSize, Epochs: integer; pGetTrainingProc,
  pGetValidationProc, pGetTestProc: TNNetGet2VolumesProc);
var
  vGetTrainingPair, vGetValidationPair, vGetTestPair: TNNetGetPairFn;
...
begin
  ...
  FBatchSize := pBatchSize;
  FMaxEpochs := Epochs;
  maxForm1.memo2.lines.Add('Debug Fit Epochs: '+inttostr(Epochs));
  ...
  maxForm1.memo2.lines.Add('Debug Fit Batchsize: '+inttostr(FBatchSize));
  FProcs := TNeuralThreadList.Create(FThreadNum);
  FStepSize := FBatchSize;
  maxForm1.memo2.lines.Add('Debug Fit stepsize: '+inttostr(FStepSize));
  if Assigned(FThreadNN) then FThreadNN.Free;
  FThreadNN := TNNetDataParallelism.Create(FNN, FThreadNum);
  maxForm1.memo2.lines.Add('Debug Fit TNNetDataParallelism: '+inttostr(FThreadNum));
  ...
  maxForm1.memo2.lines.Add('Debug Fit Hyperparams init: '+floattostr(TrainingError));
  ...
  begin
    MessageProc(
      ...
      ' Batch size:' + IntToStr(FBatchSize) +
      ' Step size:' + IntToStr(FStepSize) +
      ' Staircase epochs mX4:' + IntToStr(FStaircaseEpochs)
    );
  ...
  if FVerbose then
  begin
    MessageProc('Image mX4 Fit Computing starts...');
  end;
  if Assigned(FOnStart) then FOnStart(Self);
  globalStartTime := Now();
  while ( (FMaxEpochs > FCurrentEpoch) and Not(FShouldQuit) ) do
  begin
    ...
    FMessageProc(
      ...
      IntToStr(FTrainingSampleProcessedCnt.Size) +
  ...
  FreeAndNil(FThreadNN);
  CloseFile(CSVFile);
  FMessageProc('CAI maXbox Neural Fit Finished.');
  FRunning := false;
end;
