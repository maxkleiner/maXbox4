...
uses HttpApp, AlFcnMisc, Strutils;
     //AlFcnString;
...
begin
  ...
  for i := 0 to aParams.Count - 1 do begin
    ...
    if EncodeParams then begin
      if P > 0 then Query := Query + HTTPEncode(AlCopyStr(Str, 1, P-1)) + '=' + HTTPEncode(AlCopyStr(Str, P+1, MAXINT)) + IfThen(i < aParams.Count - 1, '&')
      else Query := Query + HTTPEncode(Str) + IfThen(i < aParams.Count - 1, '&')
    end
    else begin
      if P > 0 then Query := Query + AlCopyStr(Str, 1, P-1) + '=' + AlCopyStr(Str, P+1, MAXINT) + IfThen(i < aParams.Count - 1, '&')
      else Query := Query + Str + IfThen(i < aParams.Count - 1, '&')
    end;
  end;
  if Query <> '' then begin
    P := ALpos('?', aUrl);
