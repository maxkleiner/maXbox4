...
uses SysUtils, math, alfcnmisc, masks, strutils; //maskutils;
     //AlFcnString;
...
begin
  ...
  s := '';
  while I <= aLength do begin
    S := S + AnsiChar(StrToInt(ansiChar('$') + ansiChar(aPDUMessage[I]) + ansiChar(aPDUMessage[I+1])));
    inc(I,2); //
  end;
...
Begin
  aStart := GetTickCount;
  aResponse := Uppercase(GetResponse);
  While (pos('>'#32, aResponse) <= 0) do Begin
    if (aStart + Ftimeout) < GetTickCount then Raise EAlException.Create('Timeout!')
    else if (pos(#13#10'ERROR'#13#10, aResponse) > 0) then raise EAlException.Create(ErrorMsg + ' (' + Trim(StringReplace(aResponse, #13#10, ' ', [rfReplaceALL])) + ')' );
    aResponse := aResponse + Uppercase(GetResponse);
  end;
end;
...
Begin
  aStart := GetTickCount;
  Response := Uppercase(GetResponse);
  While (pos(#13#10'OK'#13#10, Response) <= 0) do Begin
    if (aStart + Ftimeout) < GetTickCount then Raise EAlException.Create('Timeout!')
    else if (pos(#13#10'ERROR'#13#10, Response) > 0) or ALMatchesMask(Response, '*'#13#10'+CMS ERROR: *'#13#10'*') then begin
    ...
    Response := Response + Uppercase(GetResponse);
  end;
end;
