...
uses
  Classes, SysUtils, Types, variants; //, JS;   JSValue = variant;
...
Type
  ...
  Public
    ...
    procedure Terminate1(AExitCode: Integer); virtual; //overload;
    // Extra methods.
    ...
    function CheckOptions1(Const ShortOptions: String; Const Longopts: Array of string;
      Opts,NonOpts: TStrings; AllErrors: Boolean = False): String;
    function CheckOptions2(Const ShortOptions: String; Const Longopts: TStrings;
      AllErrors: Boolean = False): String;
    function CheckOptions3(Const ShortOptions: String; Const LongOpts: Array of string;
      AllErrors: Boolean = False): String;
    function CheckOptions4(Const ShortOptions: String; Const LongOpts: String;
      AllErrors: Boolean = False): String;
    function GetNonOptions(Const ShortOptions: String; Const Longopts: Array of string): TStringDynArray;
    procedure GetNonOptions1(Const ShortOptions: String; Const Longopts: Array of string;
      NonOptions: TStrings);
    procedure GetEnvironmentList(List: TStrings; NamesOnly: Boolean); virtual; abstract;
    procedure GetEnvironmentList1(List: TStrings); virtual;
    procedure Log(EventType: TEventType; const Msg: String);
    procedure Log1(EventType: TEventType; const Fmt: String; const Args: Array of const);
    // Delphi properties
...
procedure TCustomApplication.Terminate1(AExitCode: Integer);
begin
  FTerminated:=True;
  ExitCode:=AExitCode;
end;
...
begin
  Result:=GetOptionValue1(' ',S);
end;

function TCustomApplication.GetOptionValue1(const C: Char; const S: String
  ): String;
...
function TCustomApplication.HasOption1(const C: Char; const S: String): Boolean;
...
function TCustomApplication.CheckOptions1(const ShortOptions: String;
...
function TCustomApplication.CheckOptions2(const ShortOptions: String;
  const Longopts: TStrings; AllErrors: Boolean): String;
begin
  Result:=CheckOptions(ShortOptions,LongOpts,Nil,Nil,AllErrors);
end;

function TCustomApplication.CheckOptions3(const ShortOptions: String;
...
begin
  ...
  Try
    For I:=0 to High(LongOpts) do
      L.Add(LongOpts[i]);
    Result:=CheckOptions2(ShortOptions,L,AllErrors);
  Finally
    L.Destroy;
  end;
end;

function TCustomApplication.CheckOptions4(const ShortOptions: String;
...
begin
  ...
  try
    GetNonOptions1(ShortOptions,LongOpts,No);
...
procedure TCustomApplication.GetNonOptions1(const ShortOptions: String;
...
begin
  S:=CheckOptions1(ShortOptions,LongOpts,Nil,NonOptions,true);
  if (S<>'') then
    Raise EListError.Create(S);
end;

procedure TCustomApplication.GetEnvironmentList1(List: TStrings);
begin
...
procedure TCustomApplication.Log1(EventType: TEventType; const Fmt: String;
  const Args: array of const);
begin
  try
    Log(EventType, sysutils.Format(Fmt, Args));
  except
    On E: Exception do
