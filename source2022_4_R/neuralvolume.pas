...
type
  ...
  //private
    //function add(avolume: TNNetVolume): integer;
    //function capacity: integer;
    //function count: integer;
    public
      FreeObjects: boolean;
      function count2: integer;
      function capacity2: integer;
      function count: integer;
      function capacity: integer;
      //function addnetvolume(avolume: TNNetVolume): integer;
  ...
  public
    ...
    FData: TNeuralFloatArray; //array of T;
  ...
  TNNetVolumePairList = class (TNNetList)
    ...
    public
      ...
      function addnetvolumepair(avolume:TNNetvolumePair): integer;
  end;
  {$ENDIF}
  ...
  procedure TestKMeans2();

implementation

uses
  Math, neuralbit, dialogs, fMain;
...
begin
  ...
  Showmessage(' TestTNNetVolume Testing size: '+ inttostr(TestSize));
  ...
  maxForm1.memo2.lines.Add('Fill/Inner sum:'+floattostr( (R.GetSum() - 2*TestSize)));
  ...
  maxForm1.memo2.lines.Add(' A + B:'+floattostr(Result));
  ...
  begin
    ...
    begin
      ...
      maxForm1.memo2.lines.Add(' A + 3B ERROR: '+inttostr(I)+' : '+floattostr(Aux)+
                             ' :: '+floattostr(R.Raw[I])+ ' ?= '+floattostr( A.Raw[I])+ ' + 3*'+floattostr( B.Raw[I] ));
    end;
  end;
  maxForm1.memo2.lines.Add(' A + 3B: '+floattostr(Result));
  ...
  maxForm1.memo2.lines.Add(' 2A + 3B: '+floattostr(Result));
  ...
  maxForm1.memo2.lines.Add('R += A * B:: '+floattostr(Result));

  Showmessage('Channel testing:');
  A.Resize(32, 32, 3);
  A.Fill(0);
  B.Resize(1, 1, 3);
  ...
  *)
  ...
  Showmessage('Interleave testing:');
  A.Resize(1,1,12);
  A.FillForDebug();
  A.Mul(100);
  ...
  maxForm1.memo2.lines.Add('DeInterleave B With X 2:') ;
  R.Print();
  A.Resize(1,1,128);
  B.Resize(A);
  A.FillForDebug();
  B.Copy(A);
  maxForm1.memo2.lines.Add('Pearson Correlation (A,A):'+ floattostr(A.PearsonCorrelation(B)) );
  B.Mul(-1);
  maxForm1.memo2.lines.Add('Pearson Correlation (A,-A):'+floattostr( A.PearsonCorrelation(B)) );
  B.Randomize();
  maxForm1.memo2.lines.Add('Pearson Correlation (A,Random):'+floattostr( A.PearsonCorrelation(B)) );
  R.Free;
  B.Free;
  A.Free;
  maxForm1.memo2.lines.Add('TestTNNetVolume has kindly finished.');
end;
...
begin
  ...
  //WriteLn('Testing KMeans - Clusters:', Clusters, ' Cluster Size:', ClusterSize,
    //' Samples:', Samples);
   Showmessage('Testing KMeans - Clusters: '+inttostr( Clusters) + ' Cluster Size: '+inttostr(ClusterSize) +
    ' Samples: '+ inttostr(Samples));
    maxform1.Memo2.Lines.add('Testing KMeans - Clusters: '+inttostr( Clusters) + ' Cluster Size: '+inttostr(ClusterSize) +
    ' Samples: '+ inttostr(Samples));
  ...
  Showmessage(inttostr(ClustersWithElements)+' clusters have '+inttostr( ClusteredElements)+
    ' elements. KMeans testing has finished.');
    maxform1.Memo2.Lines.add(inttostr(ClustersWithElements)+' clusters have '+inttostr( ClusteredElements)+
    ' elements. KMeans testing has finished.');
  KMeans.Free;
end;
...
begin
  ...
  //WriteLn('Testing KMeans - Clusters:', Clusters, ' Cluster Size:', ClusterSize,
    //' Samples:', Samples);
    maxform1.Memo2.Lines.add('Testing KMeans2 - Clusters: '+inttostr( Clusters) + ' Cluster Size: '+inttostr(ClusterSize) +
    ' Samples: '+ inttostr(Samples));
  ...
  begin
    ...
    maxform1.Memo2.Lines.add(inttostr(ClustersWithElements)+' clusters have '+inttostr( ClusteredElements)+
    ' elements. KMeans2 testing has finished.');
  KMeans.Free;
end;
...
function TNNetVolumeList.addnetvolume(avolume: TNNetvolume): integer;
begin
 result:= inherited add(avolume)
end;
...
begin
  ...
  begin
    maxform1.Memo2.Lines.add('No color encoding has been found:'+inttostr( color_encoding));
  end;
end;
...
begin
  ...
  begin
    maxform1.Memo2.Lines.add('Bad color encoding:'+inttostr( color_encoding));
  end;
end;
...
begin
  ...
  maxform1.Memo2.Lines.add('('+inttostr(SizeX)+','+inttostr(SizeY) +','+inttostr(Depth)+') - ');
  for I := 0 to vHigh do
  begin
    AuxData := FData[I];
    maxform1.Memo2.Lines.add(FloatToStr(AuxData)+' ');
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        maxform1.Memo2.Lines.add(inttostr(CX)+' '+inttostr(CY)+' '+inttostr(CD));
      end;
    end;
  end;
end;
...
begin
  ...
  maxform1.Memo2.Lines.add('Min: '+floattostr(GetMin())+' Max:'+floattostr(GetMax())+
                     ' Avg:'+floattostr(GetAvg())+' Non Zero:'+floattostr(GetNonZero())+' Size:'+inttostr( FSize));
end;
...
begin
  ...
  begin
    ...
    maxform1.Memo2.Lines.add('['+inttostr(CntD)+':'+floattostr(AuxMin)+' '+
                               floattostr(AuxMax)+' '+floattostr(AuxAvg)+']');
  end;
...
begin
  ...
  //maxForm1.memo2.lines.Add('Debug TVolume loadingneuron from stringlistcount: '+inttostr(S.count));
  ...
  begin
  maxForm1.memo2.lines.Add('Error while loading neuron from string. ');
   (* WriteLn
    (
...
{ TNNetList }
(*
function TNNetList.add(avolume: TNNetVolume): integer;
begin
    Result := FCount;

end;  *)
 (*
function TNNetList.addnetvolume(avolume: TNNetVolume): integer;
begin
...
function TNNetList.capacity: integer;
begin
  result:= inherited Capacity; //inherited capacity;
end;

function TNNetList.capacity2: integer;
begin
  result:= inherited capacity;
end;

function TNNetList.count: integer;
begin
  result:= inherited Count; //inherited count;
end;

function TNNetList.count2: integer;
begin
...
function TNNetVolumePairList.addnetvolumepair(
  avolume: TNNetvolumePair): integer;
begin
  result:= inherited add(avolume);
end;
