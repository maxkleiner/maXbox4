...
uses SysUtils, AlFcnMisc, Strutils,
     //AlFcnString,
     AlFcnMime;
...
begin
  ...
  Try
    aLst.Text := StringReplace(aHeaderLine,';',#13#10,[RfReplaceAll]);
    For i := 0 to aLst.Count - 1 do
      If LowerCase(Trim(aLst.Names[i])) = aName then begin
...
begin
  Result := '';
  aLowerCaseName := LowerCase(aName);
  aHeaderLine := StringReplace(aHeaderLine, #13#10, ' ', [RfReplaceAll]);
  aLst := TALStringList.Create;
  Try
    aFlag := False;
    aLst.Text := StringReplace(aHeaderLine,';',#13#10,[RfReplaceAll]);
    For i := 0 to aLst.Count - 1 do
      If LowerCase(Trim(aLst.Names[i])) = aLowerCaseName then begin
...
begin
  Result := '---------------------------' + FormatDateTime('mmddyyhhnnsszzz', Now, FormatSettings);
end;
...
begin
  ...
  P1 := Pos('--'+aBoundary+#13#10, aDataStr);
  aFlag := P1 > 0;
  Dec(P1,2);
