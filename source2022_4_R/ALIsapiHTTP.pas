...
uses Windows,
     DateUtils,
     ALMultiPartBaseParser, AlFcnMisc;
     //AlFcnString;
...
begin
  aMethodStr := Method;
       if SameText(aMethodStr, 'GET')    then result := HTTPmt_Get
  else if SameText(aMethodStr, 'POST')   then result := HTTPmt_Post
  else if SameText(aMethodStr, 'PUT')    then result := HTTPmt_Put
  else if SameText(aMethodStr, 'HEAD')   then result := HTTPmt_Head
  else if SameText(aMethodStr, 'TRACE')  then result := HTTPmt_Trace
  else if SameText(aMethodStr, 'DELETE') then result := HTTPmt_Delete
...
begin
  if Realm <> '' then Result := Format('%s Realm=%s', [WWWAuthenticate, Realm])
...
var StatusString: AnsiString;
  ...
  begin
    if Item <> '' then Headers := Headers + Format(FormatStr, [Item]);
  end;

begin
  if HTTPRequest.ProtocolVersion <> '' then begin
    if (ReasonString <> '') and (StatusCode > 0) then StatusString := Format('%d %s', [StatusCode, ReasonString])
    ...
    if Expires > 0 then AddHeaderItem(Format(FormatDateTime('"%s", dd "%s" yyyy hh:nn:ss "GMT"',
                                                                Expires,
                                                                ALDefaultFormatSettings),
                                               [CAlRfc822DayOfWeekNames[DayOfWeek(Expires)],
                                                CALRfc822MonthOfTheYearNames[MonthOf(Expires)]]),
                                      'Expires: %s'#13#10);
    if LastModified > 0 then AddHeaderItem(Format(FormatDateTime('"%s", dd "%s" yyyy hh:nn:ss "GMT"',
                                                                     LastModified,
                                                                     ALDefaultFormatSettings),
    ...
    if (Content <> '') or (ContentStream <> nil) then AddHeaderItem(IntToStr(ContentLength), 'Content-Length: %s'#13#10);
