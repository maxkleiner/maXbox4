...
unit ALHttpClient2;                  //test framework for IX  with backup functions
...
type
  ...
  {--protocol version--}
  TALHTTPProtocolVersion2 = (HTTPpv_1_0, HTTPpv_1_1);

  {--Request method--}
  TALHTTPMethod2 = (HTTPmt_Get,
                   HTTPmt_Post,
                   HTTPmt_Head,
                   HTTPmt_Trace,
                   HTTPmt_Put,
  ...
  {--Request header--}
  TALHTTPRequestHeader2 = Class(TObject)
  Private
    fHeaders: TALStrings;
    FCookies: TALStrings;
  ...
  {--TALHTTPCookie--}
  TALHTTPCookie2 = class(TCollectionItem)
  private
    FName: AnsiString;
    FValue: AnsiString;
    FPath: AnsiString;
  ...
  {--TALCookieCollection--}
  TALHTTPCookieCollection2 = class(TCollection)
  private
  protected
    function GetCookie(Index: Integer): TALHTTPCookie2;
    procedure SetCookie(Index: Integer; Cookie: TALHTTPCookie2);
  public
    function Add: TALHTTPCookie2; overload;
    function Add(const Name: AnsiString;
                 ...
                 const HttpOnly: Boolean): TALHTTPCookie2; overload;
    property Items[Index: Integer]: TALHTTPCookie2 read GetCookie write SetCookie; default;
  end;

  {--Response header--}
  TALHTTPResponseHeader2 = Class(TObject)
  Private
    ...
    FCookies: TALHTTPCookieCollection2;
  ...
  public
    ...
    Property Cookies: TALHTTPCookieCollection2 read FCookies;
  ...
  {---------------------------------------}
  TALHTTPClientProxyParams2 = Class(Tobject)
  Private
  ...
  {----------------------------}
  TALHTTPClient2 = class(TObject)
  private
    FProxyParams: TALHTTPClientProxyParams2;
    FRequestHeader: TALHTTPRequestHeader2;
    FProtocolVersion: TALHTTPProtocolVersion2;
  ...
  public
    ...
    property  ProxyParams: TALHTTPClientProxyParams2 read FProxyParams;
    property  RequestHeader: TALHTTPRequestHeader2 read FRequestHeader;
    Property  ProtocolVersion: TALHTTPProtocolVersion2 read FProtocolVersion write FProtocolVersion default HTTPpv_1_1;
...
Function  AlInternetCrackUrl2(const aUrl: AnsiString;
                             Var SchemeName,
                                 HostName,
                                 UserName,
                                 Password,
...
Function  AlInternetCrackUrl2(const aUrl: AnsiString;
                             ...
                             var PortNumber: integer): Boolean; overload;
Function  AlInternetCrackUrl2(var Url: AnsiString; // if true return UrlPath
                             var Anchor: AnsiString;
                             const Query: TALStrings): Boolean; overload;
Function  AlRemoveAnchorFromUrl2(aUrl: AnsiString; Var aAnchor: AnsiString): AnsiString; overload;
Function  AlRemoveAnchorFromUrl2(const aUrl: AnsiString): AnsiString; overload;
{$IFDEF MSWINDOWS}
function  AlCombineUrl2(const RelativeUrl, BaseUrl: AnsiString): AnsiString; overload;
Function  AlCombineUrl2(const RelativeUrl,
                             BaseUrl,
                             Anchor: AnsiString;
...
uses
  ...
  DateUtils,
  SysConst,
  Math, AlFCnMisc, OverbyteIcsUtils, //JclDateTime,
  //system.AnsiStrings,
  AlhttpCommon, Strutils;
  //ALString;
...
const ALNullDate = -700000;

{********************************************************}
constructor TALHTTPCookie2.Create(Collection: TCollection);
begin
...
{**************************************************}
procedure TALHTTPCookie2.AssignTo(Dest: TPersistent);
begin
  if Dest is TALHTTPCookie2 then
    with TALHTTPCookie2(Dest) do begin
      Name := Self.FName;
      Value := Self.FValue;
      Path := Self.FPath;
      Domain := Self.FDomain;
...
{************************************************}
function TALHTTPCookie2.GetHeaderValue: AnsiString;
begin
  Result := Format('%s=%s; ', [AlHTTPEncode(FName), AlHTTPEncode(FValue)]);
  ...
  if SameSite <> '' then Result := Result + Format('SameSite=%s; ', [SameSite]);
...
procedure TALHTTPCookie2.SetHeaderValue(Const aValue: AnsiString);
...
{**************************************************}
function TALHTTPCookieCollection2.Add: TALHTTPCookie2;
begin
  Result := TALHTTPCookie2(inherited Add);
end;

{**********************************************************}
function TALHTTPCookieCollection2.Add(const Name: AnsiString;
                                     const Value: AnsiString;
                                     const Path: AnsiString;
                                     const Domain: AnsiString;
                                     const Expires: int64; // unixDateTime format
                                     const SameSite: AnsiString;
                                     const Secure: Boolean;
                                     const HttpOnly: boolean): TALHTTPCookie2;
begin
  result := TALHTTPCookie2(inherited Add);
  result.Name := Name;
  result.Value := Value;
...
{************************************************************************}
function TALHTTPCookieCollection2.GetCookie(Index: Integer): TALHTTPCookie2;
begin
  Result := TALHTTPCookie2(inherited Items[Index]);
end;

{*********************************************************************************}
procedure TALHTTPCookieCollection2.SetCookie(Index: Integer; Cookie: TALHTTPCookie2);
begin
  Items[Index].Assign(Cookie);
end;

{***************************************}
constructor TALHTTPResponseHeader2.Create;
begin
  inherited;
  FCustomHeaders := TALStringList.create;
  FCustomHeaders.NameValueSeparator := ':';
  FCookies := TALHTTPCookieCollection2.Create(TALHTTPCookie2);
  clear;
end;

{***************************************}
destructor TALHTTPResponseHeader2.Destroy;
begin
...
{************************************}
procedure TALHTTPResponseHeader2.Clear;
begin
  FAcceptRanges:= '';
  FAge:= '';
  FAllow:= '';
...
{**********************************************************}
function TALHTTPResponseHeader2.GetRawHeaderText: AnsiString;
begin
  result := FRawHeaderText;
end;

{*********************************************************************************}
procedure TALHTTPResponseHeader2.SetRawHeaderText(Const aRawHeaderText: AnsiString);
...
{**************************************}
constructor TALHTTPRequestHeader2.Create;
Begin
  inherited;
  fHeaders:= TALStringList.create;  //TALNVStringlist
...
{**************************************}
destructor TALHTTPRequestHeader2.Destroy;
begin
...
{***********************************}
procedure TALHTTPRequestHeader2.Clear;
begin
  fHeaders.clear;
  FCookies.Clear;
end;
...
{********************************************************************************************}
function TALHTTPRequestHeader2.GetHeaderValueByPropertyIndex(const Index: Integer): ansiString;
begin
  Case index of
...
{**********************************************************************************************************}
procedure TALHTTPRequestHeader2.SetHeaderValueByPropertyIndex(const Index: Integer; const Value: AnsiString);
begin
  Case index of
...
{*********************************************************}
Function TALHTTPRequestHeader2.GetRawHeaderText: AnsiString;
...
begin
  ...
  If FCookies.Count > 0 then begin
    LValue := StringReplace(ALTrim(FCookies.text), #13#10, '; ', [rfReplaceAll]);
    if LValue <> '' then
...
{***********************************************************************************************}
procedure TALHTTPRequestHeader2.setHeaderValue(const aName: ansiString; const aValue: ansiString);
begin
  If (aName <> '') then begin
    if SameText(aName, 'Cookie') then begin
      FCookies.Clear;
...
{********************************************************************************}
procedure TALHTTPRequestHeader2.SetRawHeaderText(const aRawHeaderText: AnsiString);
...
procedure ALExtractHTTPFields(Separators,
                              ...
                              StripQuotes: Boolean = False);
begin
  AlhttpCommon.ALExtractHeaderFields(Separators,
                        WhiteSpace,
                        Quotes,
                        Content,
                        Strings,
...
begin
  ...
  if P1 > 0 then begin
    ...
    if PortNumber = 0 then begin
      ...
      else if SameText(SchemeName, 'ws') then PortNumber := 80
      else if SameText(SchemeName, 'wss') then PortNumber := 443
...
var LExtraInfo: AnsiString;
    P1: integer;
begin
  Result := AlInternetCrackUrl2(aUrl,
                               SchemeName,
                               HostName,
                               UserName,
                               Password,
...
{***********************************************}
Function  AlInternetCrackUrl2(var Url: AnsiString;  // if true return the relative url
                             var Anchor: AnsiString;
...
begin
  ...
  Result := AlInternetCrackUrl2(Url,
                               SchemeName,
                               HostName,
                               UserName,
                               Password,
...
{*****************************************************************}
Function AlRemoveAnchorFromUrl2(const aUrl: AnsiString): AnsiString;
var LAnchor: AnsiString;
begin
  result := AlRemoveAnchorFromUrl2(aUrl,LAnchor);
end;
...
begin
  result := IcsDateTimeToUTC(aLocalDateTime);
end;
...
Begin
  ...
  While (P2 > 0) and (P2 < P1) do begin
    ...
    P2 := AlPosEx('-',LDateStr,P2);
       ...
       // DD Mon YYYY HH:MM:SS +0200
       // 23 Aug 2004 06:48:46 -0700
  While Alpos('  ',LDateStr) > 0 do LDateStr := StringReplace(LDateStr,'  ',' ',[RfReplaceAll]); // DD Mon YYYY HH:MM:SS GMT
                                                                                                   // DD Mon YYYY HH:MM:SS +0200
                                                                                                   // 23 Aug 2004 06:48:46 -0700
  ...
  Try
    ...
    LLst.Text :=  StringReplace(LDateStr,' ',#13#10,[RfReplaceall]);
    ...
    LMonthLabel := ALTrim(LLst[1]); // Mon
                                    ...
                                    // Aug
    P1 := 1;
    While (p1 <= 12) and (not SameText(ALRfc822MonthOfTheYearNames[P1],LMonthLabel)) do inc(P1);
    If P1 > 12 then begin
      Result := False;
      Exit;
    end;
    ...
    LTimeZoneStr := ALTrim(LLst[4]); // GMT
                                     // +0200
                                     // -0700
    LTimeZoneStr := StringReplace(LTimeZoneStr,'(','',[]);
    LTimeZoneStr := StringReplace(LTimeZoneStr,')','',[]);
    LTimeZoneStr := Trim(LTimeZoneStr);
    If LTimeZoneStr = '' then Begin
      ...
      LTimeZoneDelta := StrToDateTime(AlCopyStr(LTimeZoneStr,2,2) + ':' + AlCopyStr(LTimeZoneStr,4,2) + ':00', LFormatSettings);
      if LTimeZoneStr[1] = '+' then LTimeZoneDelta := -1*LTimeZoneDelta;
    end
    else If SameText(LTimeZoneStr,'GMT') then  LTimeZoneDelta := 0
    else If SameText(LTimeZoneStr,'UTC') then  LTimeZoneDelta := 0
    else If SameText(LTimeZoneStr,'UT')  then  LTimeZoneDelta := 0
    else If SameText(LTimeZoneStr,'EST') then LTimeZoneDelta := StrToDateTime('05:00:00', LFormatSettings)
    else If SameText(LTimeZoneStr,'EDT') then LTimeZoneDelta := StrToDateTime('04:00:00', LFormatSettings)
    else If SameText(LTimeZoneStr,'CST') then LTimeZoneDelta := StrToDateTime('06:00:00', LFormatSettings)
    else If SameText(LTimeZoneStr,'CDT') then LTimeZoneDelta := StrToDateTime('05:00:00', LFormatSettings)
    else If SameText(LTimeZoneStr,'MST') then LTimeZoneDelta := StrToDateTime('07:00:00', LFormatSettings)
    else If SameText(LTimeZoneStr,'MDT') then LTimeZoneDelta := StrToDateTime('06:00:00', LFormatSettings)
    else If SameText(LTimeZoneStr,'PST') then LTimeZoneDelta := StrToDateTime('08:00:00', LFormatSettings)
    else If SameText(LTimeZoneStr,'PDT') then LTimeZoneDelta := StrToDateTime('07:00:00', LFormatSettings)
    else begin
      Result := False;
      Exit;
    end;
    ...
    Result := TryStrToDateTime(LDateStr,Value,LFormatSettings);
...
begin
  ...
  try
    ...
    //----------
    LLstIpv6Part.Text := StringReplace(aIPv6Str, ':', #13#10, [rfReplaceALL]);
    ...
    for I := 0 to LLstIpv6Part.Count - 1 do begin
      ...
      S1 := UpperCase(AlCopyStr(LLstIpv6Part[i], 1, 2));
      ...
      S1 := UpperCase(AlCopyStr(LLstIpv6Part[i], 3, 2));
      if (not TryStrToInt('$' + S1, P1)) or
         (not (P1 in [0..255])) then begin
        Result := False;
        exit;
...
{*******************************}
constructor TALHTTPClient2.Create;
begin
  inherited;
  ...
  FOnRedirect := nil;
  FProxyParams := TALHTTPClientProxyParams2.Create;
  FProxyParams.OnChange := OnProxyParamsChange;
  FRequestHeader := TALHTTPRequestHeader2.Create;
  FRequestHeader.UserAgent := 'Mozilla/3.0 (compatible; TALHTTPClient2)';
  FProtocolVersion := HTTPpv_1_1;
end;

{*******************************}
destructor TALHTTPClient2.Destroy;
begin
...
{***************************************************************}
procedure TALHTTPClient2.SetUsername(const NameValue: AnsiString);
begin
  FUserName := NameValue;
end;

{*******************************************************************}
procedure TALHTTPClient2.SetPassword(const PasswordValue: AnsiString);
begin
  FPassword := PasswordValue;
end;

{*****************************************************************************}
procedure TALHTTPClient2.SetOnRedirect(const Value: TAlHTTPClientRedirectEvent);
begin
  FOnRedirect := Value;
end;
...
{************************************************}
Procedure TALHTTPClient2.Get(const aUrl:AnsiString;
                            const aRequestFields: TALStrings;
                            const aResponseContent: TStream;
                            const aResponseHeader: TALHTTPResponseHeader2;
                            const ARequestHeaderValues: TALNameValueArray = nil;
...
{***********************************************}
Function TALHTTPClient2.Get(const aUrl:AnsiString;
                           const aRequestFields: TALStrings;
                           const ARequestHeaderValues: TALNameValueArray = nil;
                           Const aEncodeRequestFields: Boolean=True): AnsiString;
var LResponseContent: TStringStream;
begin
...
{**************************************************}
procedure TALHTTPClient2.Post(const aUrl: AnsiString;
                             const aPostDataStream: TStream;
                             const aResponseContent: TStream;
                             const aResponseHeader: TALHTTPResponseHeader2;
                             const ARequestHeaderValues: TALNameValueArray = nil);
...
{*************************************************}
function TALHTTPClient2.Post(const aUrl: AnsiString;
                            const aPostDataStream: TStream;
                            const ARequestHeaderValues: TALNameValueArray = nil): AnsiString;
var LResponseContent: TStringStream;
begin
...
{************************************************************}
procedure TALHTTPClient2.PostUrlEncoded(const aUrl: AnsiString;
                                       const aRequestFields: TALStrings;
                                       const aResponseContent: TStream;
                                       const aResponseHeader: TALHTTPResponseHeader2;
                                       const ARequestHeaderValues: TALNameValueArray = nil;
                                       Const aEncodeRequestFields: Boolean=True);
Var LURLEncodedContentStream: TStringStream;
    LOldRequestContentType: AnsiString;
    Str: AnsiString;
    I, P: Integer;
begin
  LURLEncodedContentStream := TStringStream.create('');
  ...
  try
    ...
    LURLEncodedContentStream:= Nil;
    FrequestHeader.ContentType := LOldRequestContentType;
  end;
end;

{***********************************************************}
function TALHTTPClient2.PostUrlEncoded(const aUrl: AnsiString;
                                      const aRequestFields: TALStrings;
                                      const ARequestHeaderValues: TALNameValueArray = nil;
                                      Const aEncodeRequestFields: Boolean=True): AnsiString;
Var LURLEncodedContentStream: TStringStream;
...
{*******************************************************************}
procedure TALHTTPClient2.PostMultiPartFormData(const aUrl: AnsiString;
                                              const aRequestFields: TALStrings;
                                              const aRequestFiles: TALMultiPartFormDataContents;
                                              const aResponseContent: TStream;
                                              const aResponseHeader: TALHTTPResponseHeader2;
...
begin
  ...
  try
    ...
    LMultipartFormDataEncoder.Free;
    LMultipartFormDataEncoder:= Nil;
    FrequestHeader.ContentType := LOldRequestContentType;
  end;
end;

{******************************************************************}
function TALHTTPClient2.PostMultiPartFormData(const aUrl: AnsiString;
                                             const aRequestFields: TALStrings;
                                             const aRequestFiles: TALMultiPartFormDataContents;
                                             const ARequestHeaderValues: TALNameValueArray = nil): AnsiString;
Var LMultipartFormDataEncoder: TALMultipartFormDataEncoder;
...
{**************************************************}
procedure TALHTTPClient2.Head(const aUrl: AnsiString;
                             const aResponseContent: TStream;
                             const aResponseHeader: TALHTTPResponseHeader2;
                             const ARequestHeaderValues: TALNameValueArray = nil);
begin
...
{************************************************}
function TALHTTPClient2.Head(const aUrl:AnsiString;
                            const ARequestHeaderValues: TALNameValueArray = nil) : AnsiString;
var LResponseContent: TStringStream;
begin
  LResponseContent := TStringStream.Create('');
...
{***************************************************}
procedure TALHTTPClient2.Trace(const aUrl: AnsiString;
                              const aResponseContent: TStream;
                              const aResponseHeader: TALHTTPResponseHeader2;
                              const ARequestHeaderValues: TALNameValueArray = nil);
begin
...
{*************************************************}
function TALHTTPClient2.Trace(const aUrl:AnsiString;
                             const ARequestHeaderValues: TALNameValueArray = nil): AnsiString;
var LResponseContent: TStringStream;
begin
  LResponseContent := TStringStream.Create('');
...
{************************************************}
procedure TALHTTPClient2.Put(const aUrl:AnsiString;
                            const aPutDataStream: TStream;
                            const aResponseContent: TStream;
                            const aResponseHeader: TALHTTPResponseHeader2;
                            const ARequestHeaderValues: TALNameValueArray = nil);
...
{************************************************}
function TALHTTPClient2.Put(const aURL: Ansistring;
                           const aPutDataStream: TStream;
                           const ARequestHeaderValues: TALNameValueArray = nil): AnsiString;
var LResponseContent: TStringStream;
begin
...
{****************************************************}
procedure TALHTTPClient2.Delete(const aUrl: AnsiString;
                               const aResponseContent: TStream;
                               const aResponseHeader: TALHTTPResponseHeader2;
                               const ARequestHeaderValues: TALNameValueArray = nil);
begin
...
{***************************************************}
function TALHTTPClient2.Delete(const aURL: Ansistring;
                              const ARequestHeaderValues: TALNameValueArray = nil): AnsiString;
var LResponseContent: TStringStream;
begin
  LResponseContent := TStringStream.Create('');
...
{****************************************************}
procedure TALHTTPClient2.Options(const aUrl: AnsiString;
                                const aResponseContent: TStream;
                                const aResponseHeader: TALHTTPResponseHeader2;
                                const ARequestHeaderValues: TALNameValueArray = nil);
begin
...
{****************************************************}
function TALHTTPClient2.Options(const aURL: Ansistring;
                               const ARequestHeaderValues: TALNameValueArray = nil): AnsiString;
var LResponseContent: TStringStream;
begin
  LResponseContent := TStringStream.Create('');
...
{*****************************************************************************************}
procedure TALHTTPClient2.OnProxyParamsChange(sender: Tobject; Const PropertyIndex: Integer);
begin
 //virtual
end;

{***************************************}
procedure TALHTTPClientProxyParams2.Clear;
begin
  FProxyBypass := '';
  FproxyServer := '';
  FProxyUserName := '';
...
{******************************************}
constructor TALHTTPClientProxyParams2.Create;
Begin
  inherited create;
  FProxyBypass := '';
  FproxyServer := '';
...
{******************************************************************}
procedure TALHTTPClientProxyParams2.DoChange(propertyIndex: Integer);
begin
  if assigned(FonChange) then FonChange(Self,propertyIndex);
end;

{*************************************************************************}
procedure TALHTTPClientProxyParams2.SetProxyBypass(const Value: AnsiString);
begin
...
{***************************************************************************}
procedure TALHTTPClientProxyParams2.SetProxyPassword(const Value: AnsiString);
begin
...
{********************************************************************}
procedure TALHTTPClientProxyParams2.SetProxyPort(const Value: integer);
begin
...
{*************************************************************************}
procedure TALHTTPClientProxyParams2.SetProxyServer(const Value: AnsiString);
begin
...
{***************************************************************************}
procedure TALHTTPClientProxyParams2.SetProxyUserName(const Value: AnsiString);
begin
