...
unit neuraldatasets;
{
  ...
  https://sourceforge.net/projects/cai/   from writeln to maxForm1.memo2.lines.Add('
}
...
uses
  ...
  neuralvolume, neuralnetworkCAI, extctrls, graphics
  {$IFDEF FPC},
...
type
  ...
  //TClassesAndElements = class(TStringStringListVolume)
   TClassesAndElements = class(TStringList)
...
procedure LoadImageIntoVolume2(M: TImage; Vol:TNNetVolume);
  procedure LoadVolumeIntoImage2(Vol:TNNetVolume; M: TImage);
...
uses
  SysUtils, math, neuralthread, fmain,
  ...
  begin
    ...
    begin
      //Result += Self.List[ClassId].Count;
       //Result := result+ TStringList(ClassId).Count;
       Result := Tstringlist(Self.objects[ClassId]).count;
    end;
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if not Assigned(TStringList(self)) then
        begin
          maxForm1.memo2.lines.Add(ClassFolder+' - error: not assigned list');
        end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        //FindAllFiles(Self.List[ClassCnt], ClassFolder, '*.png;*.jpg;*.jpeg;*.bmp', {SearchSubDirs} false);
        //Self.List[ClassCnt].FixObjects();
        ElementCnt := TStringlist(classcnt).Count;
...
begin
  ...
  //Result := TStringlist(ClassId).Strings[ElementId];
  Result := Tstringlist(Self.objects[ClassId]).Strings[ElementId];
end;
...
begin
  ...
  begin
    ...
    begin
      MaxImage := TStringlist(ClassId).Count - 1;
      if MaxImage >= 0 then
      begin
        for ImageId := 0 to MaxImage do
        begin
          //SourceVolume := Self.Strings[ClassId].strings[ImageId];
...
begin
  ...
  begin
    ...
    begin
     // MaxFileId := Self.TStringlist(objects[ClassId]).Count - 1;
      if MaxFileId >= 0 then
      begin
...
begin
  ...
  begin
    ...
    for ClassId := 0 to MaxClass do
    begin
      MaxImage := TStringlist(Self.Objects[ClassId]).Count - 1;
      if MaxImage >= 0 then
      begin
        for ImageId := 0 to MaxImage do
        begin
          //SourceVolume := TNNetvolume(Self.objects[ClassId]).tags[ImageId];
...
begin
  result := TStringlist(Self.objects[ClassId]).Count;    //fixes
end;
...
begin
  M := TImage.Create(nil);
  result:= false;
  m.SetBounds(0,0,V.SizeX, V.SizeY);
  {Result := } M.picture.LoadFromFile( ImageFileName );
  {if Result then }LoadImageIntoVolume2(M, V);
  result:= true;
  M.Free;
end;
...
var
   ...
   M: TImage;
begin
  //M := TImage.Create(V.SizeX, V.SizeY);
  M := TImage.Create(nil);
  m.SetBounds(0,0,V.SizeX, V.SizeY);
  LoadVolumeIntoImage2(V, M);
  M.picture.SaveToFile(ImageFileName);
  M.Free;
end;
    ...
    begin
      ...
      begin
        ...
        begin
          for ImageId := 0 to MaxImage do
          begin
            //SourceVolume := TNNetVolume(Self.objects[ClassId]).[ImageId];
...
begin
 ...
 {
procedure LoadImageIntoVolume(M: TImage; Vol:TNNetVolume);
var
...
//https://delphi-kb.blogspot.com/2006/10/how-to-get-rgb-value-of-pixel-under.html
...
begin
  ...
  begin
    ...
    begin
      //LocalColor := M.Colors[CountX, CountY];
       colnumb := M.Canvas.Pixels[countX, countY]; {The image can't be a JPG}
      GetRGB(colnumb, R,G,B);
      //LocalColor:= rgbtocolor(r,g,b);
      RawPos := Vol.GetRawPos(CountX, CountY, 0);

      Vol.FData[RawPos]     := R shr 8;
      Vol.FData[RawPos + 1] := G shr 8;
      //Vol.FData[RawPos + 2] := LocalColor.blue shr 8;
      Vol.FData[RawPos + 2] := B shr 8;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      RawPos := Vol.GetRawPos(CountX, CountY, 0);
      R := round(NeuronForceMinMax(Round(Vol.FData[RawPos]),0,255));
      G:=  round(NeuronForceMinMax(Round(Vol.FData[RawPos + 1]),0,255));
      B := round(NeuronForceMinMax(Round(Vol.FData[RawPos + 2]),0, 255));
...
procedure TranslateCifar10VolumesToMachineAnimal(VolumeList: TNNetVolumeList);
var
  Volume: TNNetVolume;   x: integer;
begin
  {for Volume in TNNetVolume(pointer(VolumeList)^) do
  begin
    Volume.Tag := csMachineAnimalCifar10Pos[Volume.Tag];
  end; }
  //for Volume in VolumeList do
  for x := 0 to Volumelist.Count - 1 do begin
     //volume.tag:=  volumelist[x].Tag;
     volume:=  volumelist[x];
...
var
  ...
  AuxVolume: TNNetVolume;
begin
  if Verbose then maxForm1.memo2.lines.Add('Loading images from CIFAR-100 file "'+fileName+'" ...');
  AssignFile(cifarFile, fileName);
  Reset(cifarFile);
  ...
  CloseFile(cifarFile);
  if Verbose then maxForm1.memo2.lines.Add(' '+inttostr(ImgVolumes.Count)+' images loaded.');
end;
...
begin
  ...
  begin
    if Verbose then maxForm1.memo2.lines.Add('Labels file not found:'+ fileNameLabels);
    exit;
  end;

  if not FileExists(fileNameImg) then
  begin
    if Verbose then maxForm1.memo2.lines.Add('Image file not found:'+ fileNameImg);
    exit;
  end;
  ...
  if Verbose then
    maxForm1.memo2.lines.Add(
      ...
      'File:'+ fileName+
      ' Labels:'+ inttostr(LabelItems)+ ' Images:'+ inttostr(ImgItems)+
      ' Rows:'+ inttostr(ImgRows)+ ' Cols:'+ inttostr(ImgCols)
    );
  ...
  begin
    ...
    begin
      maxForm1.memo2.lines.Add('Error loading Label:'+inttostr( LabelByte)+' at index '+inttostr( ImgCnt));
      Vol.Free;
    end
    else
    begin
...
begin
  ...
  begin
    maxForm1.memo2.lines.Add('File Not Fount: data_batch_1.bin');
    maxForm1.memo2.lines.Add('Please download it from here: https://www.cs.toronto.edu/~kriz/cifar-10-binary.tar.gz');
...
begin
  ...
  begin
    maxForm1.memo2.lines.Add('File Not Fount: train.bin');
    maxForm1.memo2.lines.Add('Please download it from here: https://www.cs.toronto.edu/~kriz/cifar-100-binary.tar.gz');
    //TODO: automatically download file
    Result := false;
  end
  else if not (FileExists('test.bin')) then
  begin
    maxForm1.memo2.lines.Add('File Not Fount: test.bin');
    maxForm1.memo2.lines.Add('Please download it from here: https://www.cs.toronto.edu/~kriz/cifar-100-binary.tar.gz');
    //TODO: automatically download file
    Result := false;
  end;
...
begin
  ...
  begin
    maxForm1.memo2.lines.Add('File Not Fount:'+ FullFileName);
    maxForm1.memo2.lines.Add('Please download from '+ Link);
    //TODO: automatically download file
    Result := false;
    exit;
  end;
...
begin
  ...
  begin
    if I > 0 then maxForm1.memo2.lines.Add('CSVConfusion'+ ',');
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        maxForm1.memo2.lines.Add('Error: invalid output value'+floattostr(OutputValue));
        CurrentLoss := 1;
      end;
