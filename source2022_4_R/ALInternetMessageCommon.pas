...
begin
  ...
  if (decodeRealName) and
   ...
   RealName := StringReplace(RealName,'\\',#1,[rfIgnoreCase,RfReplaceAll]);
   RealName := StringReplace(RealName,'\','',[rfIgnoreCase,RfReplaceAll]);
   RealName := StringReplace(RealName,#1,'\',[rfIgnoreCase,RfReplaceAll]);
  end
  else if (ln >= 2) and
...
Begin
  Result := StringReplace(ALMakeKeyStrByGUID,'-','',[rfReplaceAll]) + '@' + ALTrim(AlGetLocalHostName);
end;
...
Begin
  ...
  If ahostname <> '' then Result := StringReplace(ALMakeKeyStrByGUID,'-','',[rfReplaceAll]) + '@' + ahostname
  ...
  while (CurrentSrcPos <= SrcLength) do begin
    ...
    If src[CurrentSrcPos]= '=' then begin
      ...
      If (CurrentSrcPos+2<=SrcLength) then Begin
        ...
        If TryStrToInt(UpperCase('$'+Entity),in1) and (in1 <= 255) and (in1 >= 0) then CopyCharToResult(AnsiChar(in1), CurrentSrcPos+3)
        ...
        If P1 = 1 then begin
          ...
          If (P2 > 0) then Begin
            ...
            If P3 > 0 then begin
              ...
              If P4 > 0 then Begin
                aEncoding := LowerCase(AlcopyStr(aEncodedText,P2+1,P3-P2-1)); //q
