...
//change TInput Toutput TInputs Toutputs to TInputbc   for maXbox476
//https://github.com/ildemartinez/Delphi-Component-for-BlockChain-Dat-Files
...
type
  TCrypto = (tcBitcoin,tcPascalCoin);
  TNetbc = (tnMainNet, tnTestNet);
  UInt32 = cardinal;   UInt8= byte;
   UInt16 = smallint;
  ...
  TInputsbc = class(TObjectList)
  private
    function GetInput(index: integer): TInputbc;
  public
    function NewInput: TInputbc;
    property items[index: integer]: TInputbc read GetInput; default;
  end;

  TOutputsbc = class(TObjectList)
  private
    function GetOutput(index: integer): TOutputbc;
  public
    function NewOutput: TOutputbc;
    property items[index: integer]: TOutputbc read GetOutput; default;
  end;
...
begin
  aBlockFile.afs := TFileStream.Create(aBlockFile.aFileName,
    fmOpenRead);
...
begin
  inputs := TInputsbc.Create;
  outputs := TOutputsbc.Create;
end;
...
function TOutputsbc.GetOutput(index: integer): TOutputbc;
begin
  result := TOutputbc(inherited items[index]);
end;

function TOutputsbc.NewOutput: TOutputbc;
begin
  result := TOutputbc.Create;
  self.Add(result);
end;

{ TInputs }

function TInputsbc.GetInput(index: integer): TInputbc;
begin
  result := TInputbc(inherited items[index]);
end;

function TInputsbc.NewInput: TInputbc;
begin
  result := TInputbc.Create;
  self.Add(result);
end;

destructor TInputbc.Destroy;
begin
  FreeMem(CoinBase);
