{-----------------------------------------------------------------------------
 ...
 History:   convert record to class cause scripts
-----------------------------------------------------------------------------}
...
type
  ...
  TPythonVersion = class (TObject) //record
  private
    FDisplayName: string;
    FHelpFile: string;
...
begin
  ...
  try
    ...
    if (Header.e_magic <> IMAGE_DOS_SIGNATURE) or (Header._lfanew = 0) then begin
       result:= false;
       exit
    end;
      //Exit(False);
...
begin
  ...
  //PythonVersion:= TPythonVersion.create;
  ...
  if Result and (PythonVersion.fSysArchitecture = '') then begin
    ...
    Result := PythonVersionFromPath(PythonVersion.InstallPath, PythonVersion);
    if Result  then
      PythonVersion.fSysArchitecture := PythonVersion.ExpectedArchitecture;
  end;
...
begin
  ...
  for I := High(PYTHON_KNOWN_VERSIONS) downto 1 do begin
     PythonVersion:= TPythonVersion.create;
  //end;
