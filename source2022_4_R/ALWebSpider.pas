  ...
  with _TAlWebSpiderHandleTagfunctExtData(ExtData^) do begin
    ...
    If handled then begin
      ...
      for i := 0 to TagParams.Count - 1 do
        If TagParams.Names[i] <> '' then Result := Result + ' ' + TagParams.Names[i] + '="'+ StringReplace(TagParams.ValueFromIndex[i],
                                                                                                             '"',
                                                                                                             '&#34;',
                                                                                                             [rfReplaceAll]) + '"'
      ...
      Try
        ...
        If (not UrlRedirect) and (not DownloadError) then begin
          ...
          if CurrentHTTPResponseContent.Size > 0 then begin
            ...
            If SameText(AnsiString(pMimeTypeFromData),'text/html') and
...
Begin
  if fSaveDirectory = '' then result := ''
    ...
    aExt := lowercase(ALGetDefaultFileExtFromMimeContentType(aContentType)); // '.htm'
...
begin
  ...
  If not fStayInStartDomain or
     (lowercase(AlExtractHostNameFromUrl(ALTrim(fStartUrl))) = lowercase(AlExtractHostNameFromUrl(RedirectedTo))) then begin
...
begin
  ...
  if aFileName <> '' then begin
    If SameText(AnsiString(pMimeTypeFromData),'text/html') then begin
...
begin
  ...
  IF not fDownloadImage and
     (
      ...
      (
       ...
       SameText(ALTrim(HtmlTagParams.Values['type']),'image')
      )
     )
    then Exit;
  ...
  If fStayInStartDomain and
     (lowercase(AlExtractHostNameFromUrl(ALTrim(fStartUrl))) <> lowercase(AlExtractHostNameFromUrl(Url))) then exit;
  ...
  If fIncludeMask <> '' then begin
    ...
    Try
      Lst.Text := ALTrim(StringReplace(FIncludeMask,';',#13#10,[RfReplaceall]));
      Flag1 := True;
  ...
  If fExcludeMask <> '' then begin
    Lst := TALStringList.Create;
    Try
      Lst.Text := Trim(StringReplace(fExcludeMask,';',#13#10,[RfReplaceall]));
      Flag1 := False;
      For i := 0 to Lst.Count - 1 do begin
        S1 := ALTrim(Lst[i]);
        If S1 <> '' then begin
...
begin
  ...
  If Url <> '' then begin
    ...
    If LocalPath = '!' then localpath := ''
      ...
      LocalPath := StringReplace(LocalPath,
                                   '\',
                                   '/',
