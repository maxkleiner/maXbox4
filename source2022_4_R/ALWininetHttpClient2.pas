...
type
  ...
  {-----------------------------------------------------}
  TALWinInetHttpInternetOpenAccessType2 = (wHttpAt_Direct,                       {Resolves all host names locally.}
  ...
  {--------------------------------------------------}
  TAlWininetHttpClientInternetOption2 = (wHttpIo_Async,                     {NOT SUPPORTED YET!
  ...
  {--------------------------------------------------------------------------------}
  TALWininetHttpClientInternetOptionSet2 = Set of TAlWininetHttpClientInternetOption2;
  ...
  TAlWinInetHTTPClientStatusEvent  = procedure(sender: Tobject; InternetStatus: DWord; StatusInformation: Dword; StatusInformationLength: DWord) of object;

  {-----------------------------------------}
  TALWinInetHTTPClient2 = class(TALHTTPClient2)
  private
    FAccessType: TALWinInetHttpInternetOpenAccessType2;
    FInternetOptions: TAlWininetHTTPClientInternetOptionSet2;
    FConnected: Boolean;
    FURL: AnsiString;
    FURLHost: AnsiString;
    FURLPath: AnsiString;
    ...
    procedure SetAccessType(const Value: TALWinInetHttpInternetOpenAccessType2);
    procedure SetInternetOptions(const Value: TAlWininetHTTPClientInternetOptionSet2);
  ...
  protected
    ...
    procedure Execute(const aUrl:AnsiString;
                      ...
                      const aResponseHeader: TALHTTPResponseHeader2); override;
    function  Send(const aRequestMethod: TALHTTPMethod2;
                   const aRequestDataStream: TStream): HINTERNET; virtual;
    procedure Receive(const aContext: HINTERNET;
                      const aResponseContent: TStream;
                      const aResponseHeader: TALHTTPResponseHeader2); virtual;
  ...
  public
    ...
    property  AccessType: TALWinInetHttpInternetOpenAccessType2 read FAccessType write SetAccessType default wHttpAt_Preconfig;
    property  InternetOptions: TAlWininetHTTPClientInternetOptionSet2 read FInternetOptions write SetInternetOptions default [wHttpIo_Keep_connection];
...
begin
  ...
  with TALWinInetHTTPClient2(dwContext) do begin

    {fire the OnStatus event}
    if Assigned(FOnStatus) then
      FOnStatus(
        TALWininetHttpClient2(dwContext),
        dwInternetStatus,
    ...
    If (dwInternetStatus = INTERNET_STATUS_REDIRECT) and Assigned(OnRedirect) then
      OnRedirect(TALWininetHttpClient2(dwContext), AnsiString(PAnsiChar(lpvStatusInformation)));

  end;
end;

{**************************************}
constructor TALWinInetHTTPClient2.Create;
begin
  inherited;
  FInetRoot := nil;
  FInetConnect := nil;
  ...
  RequestHeader.UserAgent := 'Mozilla/3.0 (compatible; TALWinInetHTTPClient2)';
  FIgnoreSecurityErrors := False;
end;

{**************************************}
destructor TALWinInetHTTPClient2.Destroy;
begin
  Disconnect;
  inherited;
end;
...
{********************************************************}
procedure TALWinInetHTTPClient2.CheckError(ErrCode: DWORD);
var ln: DWORD;
    S: AnsiString;
begin
...
{********************************************************}
procedure TALWinInetHTTPClient2.CheckError(Error: Boolean);
begin
  if Error then
...
{*************************************************************}
procedure TALWinInetHTTPClient2.SetURL(const Value: AnsiString);
Var LSchemeName,
    LHostName,
    LUserName,
    LPassword,
...
begin
  If Value <> Url then Begin
    ...
    if Value <> '' then begin
      ...
      //-----
      if SameText(LSchemeName, 'https') then LScheme := INTERNET_SCHEME_HTTPS
      else if SameText(LSchemeName, 'http') then LScheme := INTERNET_SCHEME_HTTP
...
{**********************************************************************}
procedure TALWinInetHTTPClient2.SetUsername(const NameValue: AnsiString);
begin
...
{**************************************************************************}
procedure TALWinInetHTTPClient2.SetPassword(const PasswordValue: AnsiString);
begin
...
{*************************************}
procedure TALWinInetHTTPClient2.Connect;
...
const
  AccessTypeArr: Array[TALWinInetHttpInternetOpenAccessType2] of DWord = (INTERNET_OPEN_TYPE_DIRECT,
...
{****************************************}
procedure TALWinInetHTTPClient2.Disconnect;
begin
...
{*********************************************************************}
function TALWinInetHTTPClient2.Send(const aRequestMethod: TALHTTPMethod2;
...
{****************************************************************}
procedure  TALWinInetHTTPClient2.Receive(const aContext: HINTERNET;
                                        const aResponseContent: TStream;
...
{***********************************************************}
procedure TALWinInetHTTPClient2.Execute(const aUrl:AnsiString;
                                       const aRequestMethod: TALHTTPMethod2;
                                       const aRequestDataStream: TStream;
                                       const ARequestHeaderValues: TALNameValueArray;
                                       const aResponseContent: TStream;
...
{**********************************************************************************************}
procedure TALWinInetHTTPClient2.SetAccessType(const Value: TALWinInetHttpInternetOpenAccessType2);
begin
...
{***************************************************************************************}
procedure TALWinInetHTTPClient2.SetOnStatus(const Value: TAlWinInetHTTPClientStatusEvent);
begin
  Disconnect;
  FOnStatus := Value;
end;
...
{************************************************************************************}
procedure TALWinInetHTTPClient2.SetOnRedirect(const Value: TAlHTTPClientRedirectEvent);
begin
  Disconnect;
  inherited;
end;
...
{************************************************************************************************}
procedure TALWinInetHTTPClient2.OnProxyParamsChange(sender: Tobject; Const PropertyIndex: Integer);
begin
  if (PropertyIndex = -1) or (PropertyIndex in [0, 1, 2]) then Disconnect; //clear, ProxyBypass, ProxyServer, ProxyPort
end;

{****************************************************************************************************}
procedure TALWinInetHTTPClient2.SetInternetOptions(const Value: TAlWininetHTTPClientInternetOptionSet2);
begin
