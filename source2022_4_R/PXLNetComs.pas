...
type
  ...
  private
    ...
    FPacketMessageS: TPacketMessageS;

    FOnReceive2: TReceiveEvent2;
...
{$ENDIF}
    ...
    function SocketSendDataStr(const Handle: TSocket; const Data: string; const DataSize: Integer;
      const DestHost: StdString; const DestPort: Integer): Boolean;
  ...
  public
    ...
    function SendStr(const Host: StdString; const Port: Integer; const Data: string; const Size: Integer): Boolean;
    ...
    property OnReceive2: TReceiveEvent2 read FOnReceive2 write FOnReceive2;
...
function TNetCom.SocketSendDataStr(const Handle: TSocket; const Data: string;
  const DataSize: Integer; const DestHost: StdString;
  const DestPort: Integer): Boolean;
begin
  // to do
end;
...
function TNetCom.SendStr(const Host: StdString; const Port: Integer; const Data: string; const Size: Integer): Boolean;
var
  pStr: ^string;
begin
  if (not FInitialized) or (Length(Host) <= 0) or (Data = '') or (Size <= 0) then
    Exit;
   New(pStr);
   pStr^ := data;
  Result := SocketSendData(FSocket, (pStr), Size, Host, Port);
  //result:= Send(Host,Port, pointer(Data), Size);
  if Result then
  begin
    Inc(FSentPackets);
    Inc(FBytesSent, Size);
...
begin
  ...
  if Assigned(FOnReceive) then
   ...
   if Assigned(FOnReceive2) then
    FOnReceive2(Self, FromHost, FromPort, FPacketMessageS, ReceivedBytes);
