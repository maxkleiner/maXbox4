...
//       reson it causes TListView.Items to lose all associated objects and other things happen).
 ...
 procedure SendKeysToWindow(const HWnd: Windows.HWND; const Text: string);
  ...
  function GetParentProcessName2: string;                    //47590
...
function GetParentProcessName2: string;

begin
  // Assume failure
  Result := '';
  result:= GetProcessName(GetParentProcessID(getcurrentprocessID));
end;
...
begin
    ...
    //    ProcessInfo := Default(TProcessInformation);
        //ProcessInfo := TProcessInformation;   ProcessInfo.hProcess
