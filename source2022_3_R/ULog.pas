...
begin
  ...
  try
    if assigned(FFileStream) And (logType in FSaveTypes) then begin
      //if TThread.ThreadID=MainThreadID then tid := ' MAIN:' else tid:=' TID:';
   //   s := FormatDateTime('yyyy-mm-dd hh:nn:ss.zzz',now)+tid+IntToHex(TThread.CurrentThread.ThreadID,8)+' ['+CT_LogType[logtype]+'] <'+sender+'> '+logtext+#13#10;
     s := FormatDateTime('yyyy-mm-dd hh:nn:ss.zzz',now)+tid+IntToHex(TThread.InstanceSize,8)+' ['+CT_LogType[logtype]+'] <'+sender+'> '+logtext+#13#10;
    ...
    //  P^.ThreadID :=TThread.CurrentThread.ThreadID;
      P^.ThreadID :=TThread.InstanceSize;//  .ThreadID;
...
begin
  ...
  if (FFileName<>'') then begin
    ...
    if FileExists(FFileName) then fm := fmOpenWrite + fmShareDenyWrite;
      ...
      if not FileExists(FFileName) then fm := fmCreate; // + fmShareDenyWrite;
