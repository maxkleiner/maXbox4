...
{ methods }
(*----------------------------------------------------------------------------*)
function TfconvMain.SaveCheck: Boolean;
begin
  if FModified then
  begin
...
(*----------------------------------------------------------------------------*)
procedure TfconvMain.ReadLn(var s: string; const Promote, Caption: string);
begin
  s := InputBox(Caption, Promote, s)
end;

(*----------------------------------------------------------------------------*)
procedure TfconvMain.WriteLn(const s: string);
begin
...
(*----------------------------------------------------------------------------*)
procedure TfconvMain.FormCreate(Sender: TObject);
begin
  fLoading   := false;
...
(*----------------------------------------------------------------------------*)
procedure TfconvMain.EditorDropFiles(Sender: TObject; X, Y: Integer; AFiles: TStrings);
begin
  LoadFile(AFiles[0]);
end;

(*----------------------------------------------------------------------------*)
procedure TfconvMain.AnyCommand(Sender: TObject);
begin
  ExecCmd(TComponent(Sender).Tag);
end;

(*----------------------------------------------------------------------------*)
procedure TfconvMain.ExecCmd(Cmd: Integer);
begin
  case Cmd of
    OPEN_CMD       : begin
                       if SaveCheck then
...
(*----------------------------------------------------------------------------*)
procedure TfconvMain.mnuSettingsClick(Sender: TObject);
begin
...
procedure TfconvMain.LoadFile(aFileName: String);
var
  AllowChange : Boolean;
...
procedure TfconvMain.SaveFiles;
var
  oldTab, x : Integer;
...
procedure TfconvMain.TabControl1Change(Sender: TObject);
begin
  fLoading := True;
  Try
...
procedure TfconvMain.TabControl1Changing(Sender: TObject;
  var AllowChange: Boolean);
begin
  AllowChange:= True;
...
procedure TfconvMain.EditorChange(Sender: TObject);
begin
...
procedure TfconvMain.ClearTabs(FirstToo: boolean);
var
  l, i: Integer;
begin
...
procedure TfconvMain.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  ClearTabs(true);
end;

procedure TfconvMain.DlgTextSearchFind(Sender: TObject);
begin
...
procedure TfconvMain.DoSearchReplaceText(AReplace, ABackwards: boolean);
begin
...
procedure TfconvMain.mnuFindClick(Sender: TObject);
begin
  DlgTextSearch.FindText := FSearchText;
  DlgTextSearch.Execute
end;

procedure TfconvMain.mnuFindNextClick(Sender: TObject);
begin
  DoSearchReplaceText(False, ssoBackwards in FSearchOptions);
end;

procedure TfconvMain.lboMessagesDblClick(Sender: TObject);
begin
...
function TfconvMain.GetErrorXY(const inStr: string): TBufferCoord;
var
  RowCol:string;
  Row:string;
  Col:string;
...
procedure TfconvMain.GoTo1Click(Sender: TObject);
begin
...
constructor TfconvMain.Create(aOwner: TComponent);
begin
  inherited;
...
procedure TfconvMain.ReadLastUsedList;
var
  fIni   : TIniFile;
  DumStr : string;
  Cntr   : Integer;
...
procedure TfconvMain.SaveLastUsedList;
var
  fIni   : TIniFile;
  Max, Cntr   : Integer;
begin
...
procedure TfconvMain.BuildLastUsedMenu(aMenuItem: TMenuItem);
var
  Cntr, Max : Integer;
  mnuDum : TMenuItem;
begin
...
procedure TfconvMain.mnuLastUsedClick(Sender: TObject);
begin
  if Sender is TMenuItem then
    Self.LoadFile(fLastUsed.Strings[TMenuItem(Sender).Tag - 1000]);
end;

procedure TfconvMain.EditorScroll(Sender: TObject;
  ScrollBar: TScrollBarKind);
begin
  stbMain.Panels[0].Text := Format(StatusLabel,[Editor.CaretY, Editor.CaretX]);
end;

procedure TfconvMain.EditorClick(Sender: TObject);
begin
  stbMain.Panels[0].Text := Format(StatusLabel,[Editor.CaretY, Editor.CaretX]);
end;

procedure TfconvMain.EditorKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  stbMain.Panels[0].Text := Format(StatusLabel,[Editor.CaretY, Editor.CaretX]);
end;

procedure TfconvMain.EditorKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  stbMain.Panels[0].Text := Format(StatusLabel,[Editor.CaretY, Editor.CaretX]);
end;

procedure TfconvMain.WriteSetttingsToIni(const AIni: TIniFile);
begin
