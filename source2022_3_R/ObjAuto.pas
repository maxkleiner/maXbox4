...
resourcestring
    sMethodNotFound = 'Methode %s aus Klasse %s nicht gefunden';
    sTypeMisMatch = 'Keine Übereinstimmung in Parameter %d für Methode %s';
    sInvalidDispID = 'Ungültige DispID für Parameter %d in Methode %s';
    sParamRequired = 'Parameter %d erforderlich für Methode %s';
    sMethodOver = 'Methodendefinition für %s hat mehr als %d Parameter';
    sTooManyParams = 'Zu viele Parameter für Methode %s';
...
begin
  ...
  begin
    ...
    if PushData = nil then
      if (Param^.ParamType^.Kind = tkClass) and SamePropTypeName(Param^.Name, 'SELF') then
