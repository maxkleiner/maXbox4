...
begin
  Result := 'RemObjects Pascal Script. Copyright (c) 2004-2024 by RemObjects Software & maXbox5';
end;
...
{$ENDIF}
    ...
    101: Stack.SetAnsiString(-1, SysUtils.BooltoStr(stack.GetBool(-2), True)); // bo2tostr
    102: Stack.SetAnsiString(-1, SysUtils.BooltoStr(stack.GetBool(-2), stack.GetBool(-3)));
    // botostr  //Stack.GetAnsiString(-2), Stack.GetInt(-3)
     //by max
     103: Stack.SetansiString(-1, (sysutils.InttoHex(Stack.GetInt64(-2), Stack.GetInt(-3))));
     {$C+}                                                //bugfix 3.9.6
    104: Assert(Stack.GetBool(-1), Stack.GetString(-2));
    {$C-}
    //53: Stack.SetReal(-1, Power(Stack.GetReal(-2),Stack.GetReal(-3)));
    105: Stack.SetReal(-1, Frac(Stack.GetReal(-2)));
    ...
    106: Stack.SetReal(-1, Ln(Stack.GetReal(-2)));
    //58: Stack.SetString(-1, DateTimeToStr(Stack.GetReal(-2)));
    107: Stack.SetReal(-1, SQR(Stack.GetReal(-2))); // Sqr
    108: Stack.SetReal(-1, Exp(Stack.GetReal(-2)));
    109: Stack.SetBool(-1, Odd(Stack.GetInt(-2)));
    110: Stack.setString(-1, SysUtils.DatetimetoStr(stack.getreal(-2))); // datetime
    111: Stack.SetansiString(-1, (sysutils.InttoHex(Stack.GetInt64(-2), Stack.GetInt(-3))));
    112: Stack.setString(-1, SysUtils.DatetimetoStr(stack.getreal(-2))); // datetime2
    113: Stack.setString(-1, Sysutils.Format(stack.getstring(-2), [stack.FDataPtr])); // datetime2
    114: Stack.setString(-1, gettextfromfile(stack.getstring(-2))); // datetime2
    115: Stack.setString(-1, gettextfromfile2(stack.getstring(-2))); // gettexzfromfile
    //116: PPSvariant(Stack).FType.MethodAddress(-1, System.Win.ComObj.CreateOleObject(stack.Getstring(-2)))); // datetime2
    //116: Idispatch(PPSvariant(Stack).FType.GetInterface({00020400-0000-0000-C000-000000000046},
      //              System.Win.ComObj.CreateOleObject(stack.Getstring(-2)))); // datetime2
     //116: Stack.setwidestring(-1,
       //             System.Win.ComObj.CreateOleObject(stack.Getstring(-2)));
     //116:  IDispatchInvoke(System.Win.ComObj.CreateOleObject(stack.Getstring(-2)),true,
       //                                               'createoleobject',[]);
     116:  (Stack.SetClass(-1, TObject(System.Win.ComObj.CreateOleObject(stack.Getstring(-2)))));
     117: Stack.setString(-1, gettextfromfile3(stack.getansistring(-2))); // gettexzfromfile
     118: Stack.setString(-1, sysutils.stringreplace(stack.getstring(-2),stack.getstring(-3),stack.getstring(-3),[rfReplaceAll, rfIgnoreCase])); // psstringreplace
...
begin
  ...
  {$ENDIF}
   //by max 2023
  RegisterFunctionName('BO2STR', DefProc, Pointer(101), nil);
  RegisterFunctionName('BOOLTOSTR', DefProc, Pointer(102), nil);
  RegisterFunctionName('InttoHex', DefProc, Pointer(103), nil);
RegisterFunctionName('Assert', DefProc, Pointer(104), nil);
RegisterFunctionName('Frac', DefProc, Pointer(105), nil);
RegisterFunctionName('LN', DefProc, Pointer(106), nil);
RegisterFunctionName('SQR', DefProc, Pointer(107), nil);
RegisterFunctionName('Exp', DefProc, Pointer(108), nil);
RegisterFunctionName('ODD', DefProc, Pointer(109), nil);
RegisterFunctionName('DatetimetoStr', DefProc, Pointer(110), nil);
RegisterFunctionName('Int2Hex', DefProc, Pointer(111), nil);
RegisterFunctionName('DateTime2Str', DefProc, Pointer(112), nil);
RegisterFunctionName('mFormat', DefProc, Pointer(113), nil);
RegisterFunctionName('GetTextFromFile', DefProc, Pointer(114), nil);
RegisterFunctionName('GetTextFromFile2', DefProc, Pointer(115), nil);
RegisterFunctionName('x2CreateOleObject', DefProc, Pointer(116), nil);
RegisterFunctionName('GetTextFromFile3', DefProc, Pointer(117), nil);
RegisterFunctionName('psStringReplace', DefProc, Pointer(118), nil);


 (*     103: Stack.SetString(-1, InttoHex(Stack.GetInt(-2), Stack.GetInt(-3)));
     {$C+}                                                //bugfix 3.9.6
    104: Assert(Stack.GetBool(-1), Stack.GetString(-2));
    {$C-}
    //53: Stack.SetReal(-1, Power(Stack.GetReal(-2),Stack.GetReal(-3)));
    ...
    108: Stack.SetReal(-1, Exp(Stack.GetReal(-2)));
    109: Stack.SetBool(-1, Odd(Stack.GetInt(-2)));
    110: Stack.SetAnsiString(-1, SysUtils.DatetimetoStr(stack.getreal(-2))); // datetime
    //*)
