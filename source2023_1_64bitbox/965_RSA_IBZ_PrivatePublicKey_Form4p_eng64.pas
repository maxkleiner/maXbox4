unit PrivatePublicKey_RSA_Form2_Performance_ENG_64;

//Given an RSA key (n,e,d), construct a program to encrypt and decrypt plaintext messages strings.
//Purpose: Teaching and Train RSA  , #locs:020702070                             
...
begin
  ...
  PrintF('%d %s',[Trunc((Time-Time1)*24),FormatDateTime('"h found runtime:" nn:ss:zzz',Time-Time1)])
   labelperf.caption:= format('%d %s',[Trunc((Time-Time1)*24),FormatDateTime('"h found runtime:" nn:ss:zzz',Time-Time1)]);
...
begin
 //object Main: TMain
  ...
  Caption := 'Computer Science Security by maXbox4 Scripting 64bit'
  ClientHeight := 707
  ClientWidth := 787
  Color := clMoneyGreen
  ...
  parent:= main
     ...
     with groupbox1 do begin
      ...
      with label5 do begin
       parent:= groupbox1;
        ...
        Width := 287
        Height := 13
        Caption:= 'Exponents "e" 3/17/65537 have bin. less zeros, more quality'
      end ;
    ...
    with groupbox3 do begin
      ...
      parent:= groupbox3;
        ...
        Labelperf:= TLabel.create(main)
       with labelperf do begin
       parent:= groupbox3;
         Left := 10
         Top := 110
         Width := 120
...
if LoadDFMFile2Strings('C:\maXbox\softwareschule\IBZ_2016\IBZ_IT_Security_2016\DVD Inhalt\RSA Berechnung\PrivateKey.dfm',srlist, abt)= 0 then
 ...
 { bignumbertesthex;
  ...
  //Pivot from umatrix
   println(sha1(exepath+'maXbox5.exe'));
 ...
 //function chain
  ...
  // timestamp example manipulation
    ...
    println('message decrypt chaine: '+
    RSADecrypt(RSAEncrypt(('123456'),'65537','536071'),'408473','536071')); 
